...
public class DatabaseMessages {
            ...
            if (!TextUtils.isEmpty(mSubject)) {
                ...
                if (!MmsUtils.isSubjectNeedUtf8()) {
                    mSubject = getDecodedString(
                            getStringBytes(mSubject, CharacterSets.ISO_8859_1),
                            mSubjectCharset);
                }
    ...
    public static class MmsPart implements Parcelable {
        ...
        public void load(final Cursor cursor, final boolean loadMedia) {
            ...
            try{
                mIsDrm = ContentType.isDrmType(mContentType);
                if (mIsDrm){
                    mDrmDataPath = MessagingDrmSession.get().getPath(getDataUri());
                    mDrmOrigContentType = MessagingDrmSession.get().getDrmOrigMimeType(mDrmDataPath, mContentType);
                }
            }catch(Exception ex){
                ex.printStackTrace();
            }
            Log.d(TAG, " mUri " +mUri+" dataUri "+getDataUri()+" mIsDrm "+mIsDrm+"  mDrmDataPath "+mDrmDataPath+" mDrmOrigContentType "+mDrmOrigContentType);
            if (isMedia()) {
                ...
                Log.d(TAG, " is media  load? "+loadMedia+"  mContentType "+mContentType);
                if (loadMedia) {
                    if (ContentType.isImageType(mContentType)
                        ||(mIsDrm && ContentType.isImageType(mDrmOrigContentType))) {
                        loadImage();
                    } else if (ContentType.isVideoType(mContentType)
                        ||(mIsDrm && ContentType.isVideoType(mDrmOrigContentType))) {
                        loadVideo();
        ...
        private void loadVideo() {
            ...
            try {
                ...
                // FIXME: in plantform, 3gpp will transform to mp4 by extractMetadata.
                if(!ContentType.VIDEO_3GPP.equals(mContentType)) {
                    mContentType =
        ...
        public boolean isMedia() {
            return ContentType.isImageType(mContentType)
                    ...
                    || ContentType.isVcalendarType(mContentType)
                    || ContentType.isDrmType(mContentType);
        }
