...
public class MmsSender {
    ...
    //489220 begin
    public static void sendMmsReadRec(final Context context, final int subId, final Uri messageUri,
                                      final ReadRecInd readRecInd, final Bundle sentIntentExras) throws MmsFailureException {
        sendMms(context,
                subId,
                messageUri,
                null /* locationUrl */,
    ...
    private static void sendMms(final Context context, final int subId, final Uri messageUri,
        ...
        // Send the message
        if (shoudStopMmsSending(context,pdu, subId)){
            LogUtil.d(TAG,"Invalid FDN destinationAddress");
    ...
    private  static boolean checkFdnEnable(Context context,int subid){
        TelephonyManagerEx mTelephonyManagerEx =TelephonyManagerEx.from(context);
        try {
    ...
    private static boolean shoudStopMmsSending(Context context,GenericPdu pdu , int sub) {
        EncodedStringValue[] value = null;
        if(pdu instanceof SendReq){
            value = ((SendReq)pdu).getTo();
        }
        if(null == value){
            LogUtil.w(TAG,"not SendReq");
            return false;
        }
        if(!checkFdnEnable(context,sub)){
            LogUtil.w(TAG,"checkFdnEnablefail--");
            return false;
        }
        ...
        LogUtil.w(TAG,"listContacts2.size"+listContacts.size());
        if (listContacts.size() > 0) {
            for(int i = 0; i < listContacts.size() ; i++){
                LogUtil.w(TAG,"listContacts2.get(i)"+listContacts.get(i));
                if(isInFdnList(context, listContacts.get(i), sub)){
                    return false;
                } else{
                    showFdnDialogSendHandler(listContacts.get(i));  //bug 1178144
                }
            }
        }
        return true;
    ...
    //bug 1178144 begin
    private static synchronized void showFdnDialogSendHandler(String number){
        Bundle bundle = new Bundle();
        bundle.putString("numberCode", number);
        Message msg = new Message();
        msg.what= GlobleUtil.MMS_ERROR_FDN_CHECKED_LOADING_DIALOG;
        msg.setData(bundle);
        GlobleUtil.sendMessageTakeBundle(GlobleUtil.TAG_LOADING_DIALOG,msg);
    }
    //bug 1178144 end

    private static boolean isInFdnList(Context context, String number, int subId) {
        ...
        try {
            while (cursor != null && cursor.moveToNext()) {
                ...
                Log.w("MmsSender","compareNumber"+compareNumber);
        ...
        LogUtil.w(TAG,"isInFdnList--false");
        return false;
    }
