...
public class UriUtil {
    ...
    public static Uri persistContentToScratchSpace(final InputStream inputStream) {
        ...
        LogUtil.d(LogUtil.BUGLE_TAG, "persistContentToScratchSpace scratchSpaceUri  "+scratchSpaceUri);
        return copyContent(context, inputStream, scratchSpaceUri);
    }

    public static Uri persistContentToScratchSpaceForDrm(final InputStream inputStream, final String outputFileName) {
        final Context context = Factory.get().getApplicationContext();
        final Uri scratchSpaceUri = MediaScratchFileProvider.buildMediaScratchSpaceUriForDrm(outputFileName, null);
	 LogUtil.d(LogUtil.BUGLE_TAG, "persistContentToScratchSpaceForDrm scratchSpaceUri  "+scratchSpaceUri);
        return copyContent(context, inputStream, scratchSpaceUri);
    }
    ...
    public static Uri persistContentToScratchSpace(final Uri sourceUri) {
        ...
        try {
            ...
            LogUtil.d(LogUtil.BUGLE_TAG, "Error while retrieving media ", ex);
            return null;
        } finally {
            if (inputStream != null) {
                try {
    ...
    public static Uri persistContentToScratchSpaceForDrm(final Uri sourceUri, final String outputFileName) {
        InputStream inputStream = null;
        final Context context = Factory.get().getApplicationContext();
        try {
            if (UriUtil.isLocalResourceUri(sourceUri)) {
            ...
            return persistContentToScratchSpaceForDrm(inputStream, outputFileName);
        } catch (final Exception ex) {
    ...
    public static Uri persistContent(
            final InputStream inputStream, final Uri outputUri, final String contentType) {
        if (outputUri == null) {
            return null;
        }

        final Context context = Factory.get().getApplicationContext();
        final Uri targetUri = FileUtil.getNewFile(context.getContentResolver(), outputUri, contentType);
        return copyContent(context, inputStream, targetUri);
    }

    @DoesNotRunOnMainThread
    public static Uri persistContent(
            final Uri sourceUri, final Uri outputUri, final String contentType) {
        InputStream inputStream = null;
        final Context context = Factory.get().getApplicationContext();
        try {
            if (UriUtil.isLocalResourceUri(sourceUri)) {
            ...
            return persistContent(inputStream, outputUri, contentType);
        } catch (final Exception ex) {
            LogUtil.e(LogUtil.BUGLE_TAG, "Error while retrieving media ", ex);
            return null;
        } finally {
    ...
    private static Uri copyContent(
        ...
        try {
            ...
            LogUtil.e(LogUtil.BUGLE_TAG, "copyContent inputStream  "+inputStream+"  outputStream"+outputStream);
    ...
    private static final String sDefaultAttachmentSavePath ="/sdcard/Messaging/";
    public static String getValidSaveAttachmentPath(){
        try {
            File external = Environment.getExternalStorageDirectory();
            String path = sDefaultAttachmentSavePath;
            if (external != null) {
                String parent = external.getAbsolutePath();
                path = parent + "/Messaging/";
                //Log.d("tony", "external path is "+path);
            }
            //Log.d("tony", "return path is "+path);
            return path;
        }catch (Exception ex){
            Log.d("UriUtil", " getValidSaveAttachmentPath ", ex.fillInStackTrace());
        }
        return sDefaultAttachmentSavePath;
    }

    public static void scannerSaveAttachment(final Context context, final Uri  file){
        try {
            if (context == null || file == null) {
                return;
            }
            Intent scanner = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE);
            scanner.setData(file);
            context.sendBroadcast(scanner);
        }catch (Exception ex){
            Log.d("UriUtil", " scannerSaveAttachment ", ex.fillInStackTrace());
        }
    }

    public static String getContentFromPartUri(final Uri uri) {
        String contentType = "";
        if (uri != null
                && uri.toString().startsWith("content://mms/part/")) {
            Cursor cursor = null;
            final String[] part_projection = new String[]{Telephony.Mms.Part.CONTENT_TYPE};
            try {
                cursor = Factory.get().getApplicationContext().getContentResolver()
                        .query(uri, part_projection, null, null, null);
