...
public class UiUtils {
    ...
    public static void showToastAtBottom(final String message) {
        Log.w("UiUtils","showToastAtBottom"+message);
        if(mToast!=null){
            mToast.cancel();
        }
    ...
    public static void showToast(final int pluralsMessageId, final int count) {
        Log.w("UiUtils","showToast2");
        if(mToast!=null){
            mToast.cancel();
        }
        mToast = Toast.makeText(getApplicationContext(),getApplicationContext().getResources().getQuantityString(pluralsMessageId, count),Toast.LENGTH_LONG);
        mToast.setGravity(Gravity.CENTER_HORIZONTAL, 0, 0);
        mToast.show();
    }
    ...
    public static boolean isReadyForAction() {
        ...
        return phoneUtils.isSmsCapable() &&
                (phoneUtils.getHasPreferredSmsSim() ||(phoneUtils.getActiveSubscriptionCount() > 1))//add 1146740 by sprd for ask-everytime  feature 
    ...
    // Add for bug 611596  start
    public interface RequestParentViewFoucsLister {
        public void onDialLogDismiss();
    }

    private static RequestParentViewFoucsLister mRequestParentViewFoucsLister;
    private static android.app.AlertDialog.Builder mRecipientLimitAlertDialog = null;   //Bug 892113
    // Add for bug 611596  end

    public static void showExceedRecipientLimitDialog(final Context context, int limit, int currentCount) {
        Activity a = getActivity(context);
        //Bug 892113 begin alertDialog-> mRecipientLimitAlertDialog
        if (a != null && mRecipientLimitAlertDialog == null) {
            mRecipientLimitAlertDialog = new AlertDialog.Builder(a);
            mRecipientLimitAlertDialog.setMessage(context.getResources().
                    getString(R.string.exceed_participant_limit, limit, currentCount - limit));
            mRecipientLimitAlertDialog.setCancelable(false);

            // Add for bug 611596  Start
            mRecipientLimitAlertDialog.setPositiveButton(
                    context.getResources().getString(R.string.confirm_convert),
                    new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            mRequestParentViewFoucsLister.onDialLogDismiss();
                            mRequestParentViewFoucsLister = null;
                            mRecipientLimitAlertDialog = null;
                        }
                    });
            // Add for bug 611596  end
            //alertDialog.setNegativeButton(context.getResources().getString(R.string.cancel_convert),null);
            if (!a.isFinishing() && !a.isDestroyed()) {
                mRecipientLimitAlertDialog.show();
            }
        }
        //Bug 892113 end
    }

    public interface ShowSaveAttachmentInfoCallback{
        public void save(final Uri uri);
        public void ingore(final Uri uri);
    }

    public static void showSaveAttachmentInfo(final Context context, final ShowSaveAttachmentInfoCallback callback, final Uri uri){
        try {
            final Activity a = getActivity(context);
            //Log.d("tony", "show dialog activity " + a);
            if (a != null) {
                a.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        AlertDialog.Builder builder = new AlertDialog.Builder(a);
                        builder.setMessage(context.getResources().getString(R.string.show_save_attachment_info));
                        builder.setPositiveButton(context.getResources().getString(com.android.internal.R.string.ok), new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                callback.save(uri);
                            }
                        });
                        builder.setNegativeButton(context.getResources().getString(com.android.internal.R.string.cancel), new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                callback.ingore(uri);
                            }
            ...
            Log.d("UiUtils", "showSaveAttachmentInfo ", ex.fillInStackTrace());
        }
    }
}
