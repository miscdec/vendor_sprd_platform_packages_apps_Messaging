...
public abstract class PhoneUtils {
    ...
    public  static void setDefaultSubid(int nSubid){
        matomicInteger.set(nSubid);
    }
    ...
    public static class PhoneUtilsLMR1 extends PhoneUtils implements LMr1 {
        ...
        public boolean hasSim() {
            try {
                ...
                LogUtil.e(TAG, "PhoneUtils.getActiveSubscriptionInfoCount: system exception for ", e);
            }
                return true;
        }
        ...
        public int getDefaultSmsSubscriptionId() {
            //Modify by 762372 begin
            if(getDefaultsubid()>=0){
                return  getDefaultsubid();
            }
            final int systemDefaultSubId = SmsManager.getDefaultSmsSubscriptionId();
            LogUtil.d(TAG,"[PhoneUtils]==getDefaultSmsSubscriptionId==systemDefaultSubId:"+systemDefaultSubId);
            setDefaultSubid(systemDefaultSubId);
            //Modify by 762372 end
        ...
        @Override
        public boolean getHasPreferredSmsSim() {
            /*Modify by SPRD for Bug:558683 Start */
            int subId = getDefaultSmsSubscriptionId();
            boolean isSimReady = mTelephonyManager.getSimState(SubscriptionManager.getPhoneId(subId)) == TelephonyManager.SIM_STATE_READY;
            LogUtil.e(TAG,"getHasPreferredSmsSim >>>>>>[" + subId + "] isSimReady:"+isSimReady);

            return  subId != ParticipantData.DEFAULT_SELF_SUB_ID && isSimReady;
            /*Modify by SPRD for Bug:558683 End */
        }
    ...
    /**
    * add by bug841310 begin
    * Check if the subId is active
    */
     public boolean isActiveSubId(int subId){
            return SubscriptionManager.from(mContext).isActiveSubId(subId);
     }
    /*add by bug841310 end*/
    ...
    public String formatForDisplay(final String phoneText) {
        ...
        */
        if (MmsSmsUtils.isPhoneNumber(phoneText)) {
            return PhoneNumberUtils.formatNumber(phoneText,phoneText,getLocaleCountry());
            }else{
            return phoneText;
            }
