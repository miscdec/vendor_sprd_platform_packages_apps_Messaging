...
public class ContactUtil {
    ...
    public static final int INDEX_DISPLAY_ACCOUNT_NAME = 9;
    /* Add by SPRD for bug 550235 End */

    /* Add by SPRD for bug 903459 start */
    public static final int INDEX_MIMETYPE = 10;
    /* Add by SPRD for bug 903459 End */
    ...
    public static class PhoneQuery {
        ...
        public static final String[] PROJECTION = new String[] {
                ...
                //Contacts.DISPLAY_ACCOUNT_NAME, // 9
                "account_name",
                "mimetype"
        };
    }
    ...
    public static class PhoneLookupQuery {
        public static final String[] PROJECTION = new String[] {
                ...
                /* Modify by SPRD for Bug:498646 2015.12.03 Start */
                // PhoneLookup.PHOTO_THUMBNAIL_URI, // 2
                PhoneLookup.PHOTO_URI, // 2
                /* Modify by SPRD for Bug:498646 2015.12.03 End */

                PhoneLookup.NUMBER, // 3
                PhoneLookup.TYPE, // 4
                PhoneLookup.LABEL, // 5
                PhoneLookup.LOOKUP_KEY, // 6
        // The data id is not included as part of the projection since it's not
        // part of
        // PhoneLookup. This is okay because the _id field serves as both the
        // data id and
        // contact id. Also we never show the results directly in a list view so
        // we are not
        // concerned about duplicated _id's (namely, the same contact has two
        // same phone
        // numbers)
        };

        public static final String[] PROJECTION_FDN = new String[] {
                // The _ID field points to the contact id of the content
                PhoneLookup._ID, // 0
                PhoneLookup.DISPLAY_NAME, // 1
                PhoneLookup.PHOTO_URI, // 2
    ...
    public static class FrequentContactQuery {
        public static final String[] PROJECTION = new String[] { Contacts._ID, // 0
    ...
    public static class SelfQuery {
        public static final String[] PROJECTION = new String[] { Profile._ID, // 0
                Profile.DISPLAY_NAME_PRIMARY, // 1
                Profile.PHOTO_THUMBNAIL_URI, // 2
                Profile.LOOKUP_KEY // 3
        // Phone number, type, label and data_id is not provided in this
        // projection since
        // Profile CONTENT_URI doesn't include this information. Also, we don't
        // need it
    ...
    public static class StructuredNameQuery {
        public static final String[] PROJECTION = new String[] {
                StructuredName.DISPLAY_NAME, StructuredName.GIVEN_NAME,
                StructuredName.FAMILY_NAME, StructuredName.PREFIX,
                StructuredName.MIDDLE_NAME, StructuredName.SUFFIX };
    }
    ...
    public static void showOrAddContact(final View view, final long contactId,
        ...
        if (contactId > ParticipantData.PARTICIPANT_CONTACT_ID_NOT_RESOLVED
            ...
            final Uri lookupUri = Contacts.getLookupUri(contactId,
                    contactLookupKey);
            ContactsContract.QuickContact.showQuickContact(view.getContext(),
                    view, lookupUri, ContactsContract.QuickContact.MODE_LARGE,
                    null);
        } else if (!TextUtils.isEmpty(normalizedDestination)
                && !TextUtils.equals(normalizedDestination,
                        ParticipantData.getUnknownSenderDestination())) {
//Bug 849992
/*
            ...
            dialog.show();
*/
                new AddContact(view.getContext(),avatarUri, normalizedDestination).execute(normalizedDestination);//Bug 849992
        }
    }

//Bug 849992
    private static class AddContact extends BugleDatabaseOperations.AsyncDestConvertTask{
        private Context mContext;
        private Uri mAvatarUri;
        private String mNormalizedDestination;
        public AddContact (Context context, final Uri avatarUri, String normalizedDestination){
            mContext = context;
        ...
        protected void onPostExecute(String result) {
            String finalDestionation = result != null ? result : mNormalizedDestination;
            //Log.d("AddContact","mNormalizedDestination: "+mNormalizedDestination+" result: "+result+" finalDestionation: "+finalDestionation);
            final AddContactsConfirmationDialog dialog = new AddContactsConfirmationDialog(
                    mContext, mAvatarUri, finalDestionation);
            try{
                dialog.show();
            }catch(Exception e){
                Log.e("AddContact","show",e);
            }
         }
     }
//Bug 849992
    ...
    public static CursorQueryData getSelf(final Context context) {
        ...
        return new CursorQueryData(context, Profile.CONTENT_URI,
                SelfQuery.PROJECTION, null, null, null);
    }
    ...
    public static CursorQueryData getPhones(final Context context) {
        ...
        // The AOSP Contacts provider allows adding a
        // ContactsContract.REMOVE_DUPLICATE_ENTRIES
        // query parameter that removes duplicate (raw) numbers. Unfortunately,
        // we can't use that
        // because it causes the some phones' contacts provider to return
        // incorrect sections.
        /*Modify by SPRD for bug584098 20160810  Start*/
        if(MmsConfig.isFilterEmail()){
            String selection = " (data.data1 IS NOT NULL OR data.data1 != '') AND (data.mimetype_id = 1 OR data.mimetype_id = 5) ";
            ...
            return new CursorQueryData(context, uri, PhoneQuery.PROJECTION, selection,
        ...
        return new CursorQueryData(context, uri, PhoneQuery.PROJECTION, null,
                null, PhoneQuery.SORT_KEY);
    }
        /*Modify by SPRD for bug584098 20160810  End*/
    }
    ...
    private static CursorQueryData filterPhonesInternal(final Context context,
        ...
        if(MmsConfig.getFdnContactFittingEnable()){
            //String flag = Integer.toString(SystemProperties.getInt("sys.messag.fdntype", 2));
            //android.util.Log.d("jinqiming","sys.messag.fdntype="+flag);
            final Uri uri = Phone.CONTENT_URI.buildUpon().appendPath("filter_fdn")
                    .appendPath("0").appendPath(query).appendQueryParameter(
                            ContactsContract.DIRECTORY_PARAM_KEY,
                            String.valueOf(Directory.DEFAULT))
                    .build();
            //android.util.Log.d("FDN","filterPhonesInternal-uri ="+uri.toString());
            return new CursorQueryData(context, uri, PhoneQuery.PROJECTION, null,
                    null, PhoneQuery.SORT_KEY);
        }else {
     ...
     */
    @VisibleForTesting
    public static CursorQueryData lookupPhone(final Context context,
            final String phone) {
        if (!ContactUtil.hasReadContactsPermission()) {
            return CursorQueryData.getEmptyQueryData();
        }
        //Modify for bug 705707 begin
        final Uri uri;
        if (MmsConfig.getFdnContactFittingEnable()) {
            Log.d("FdnContact","filter_fdn");//bug821500
            uri = Phone.CONTENT_URI.buildUpon().appendPath("filter_fdn")
                    .appendPath("0").appendPath(phone).appendQueryParameter(
                            ContactsContract.DIRECTORY_PARAM_KEY,
                            String.valueOf(Directory.DEFAULT))
                    .build();
            return new CursorQueryData(context, uri, PhoneLookupQuery.PROJECTION_FDN,
                null, null, null);//bug821500 
        } else {
            uri = getPhoneLookupUri().buildUpon().appendPath(phone)
                    .build();
        }
        //Modify for bug 705707 end
        return new CursorQueryData(context, uri, PhoneLookupQuery.PROJECTION,
                null, null, null);
    }
    ...
    public static CursorQueryData getFrequentContacts(final Context context) {
        ...
        return new FrequentContactsCursorQueryData(context,
                FrequentContactQuery.PROJECTION, null, null, null);
    }
    ...
    private static CursorQueryData filterEmailsInternal(final Context context,
        ...
        final Uri uri = Email.CONTENT_FILTER_URI
                .buildUpon()
                .appendPath(query)
                .appendQueryParameter(ContactsContract.DIRECTORY_PARAM_KEY,
                        String.valueOf(Directory.DEFAULT)).build();

        return new CursorQueryData(context, uri, EmailQuery.PROJECTION, null,
     ...
     */
    @VisibleForTesting
    public static CursorQueryData lookupEmail(final Context context,
            final String email) {
        if (!ContactUtil.hasReadContactsPermission()) {
            return CursorQueryData.getEmptyQueryData();
        }

        final Uri uri = getEmailContentLookupUri()
                .buildUpon()
                .appendPath(email)
                .appendQueryParameter(ContactsContract.DIRECTORY_PARAM_KEY,
                        String.valueOf(Directory.DEFAULT)).build();
     ...
     */
    private static CursorQueryData lookupStructuredName(final Context context,
            final long contactId, final boolean primaryOnly) {
        if (!ContactUtil.hasReadContactsPermission()) {
            return CursorQueryData.getEmptyQueryData();
        }
        ...
        final String[] selectionArgs = { StructuredName.CONTENT_ITEM_TYPE };
        if (primaryOnly) {
     ...
     */
    public static String lookupFirstName(final Context context,
        ...
        try {
            nameCursor = ContactUtil.lookupStructuredName(context, contactId,
                    true).performSynchronousQuery();
            if (nameCursor != null && nameCursor.moveToFirst()) {
                firstName = nameCursor
                        .getString(ContactUtil.INDEX_STRUCTURED_NAME_GIVEN_NAME);
            }
        } finally {
            if (nameCursor != null) {
                nameCursor.close();
    ...
    public static RecipientEntry createRecipientEntry(final String displayName,
            final int displayNameSource, final String destination,
            final int destinationType, final String destinationLabel,
            final long contactId, final String lookupKey, final long dataId,
            final String photoThumbnailUri, final boolean firstLevel) {
        if (firstLevel) {
            return RecipientEntry.constructTopLevelEntry(displayName,
                    displayNameSource, destination, destinationType,
                    destinationLabel, contactId, null, dataId,
                    photoThumbnailUri, true, lookupKey);
        } else {
            return RecipientEntry.constructSecondLevelEntry(displayName,
                    displayNameSource, destination, destinationType,
                    destinationLabel, contactId, null, dataId,
                    photoThumbnailUri, true, lookupKey);
        }
    ...
    /**
     * Creates a RecipientEntry for PhoneQuery result. The result is then
     * displayed in the contact search drop down or as replacement chips in the
     * chips edit box.
     */
    public static RecipientEntry createRecipientEntryForPhoneQuery(
        ...
        final String photoThumbnailUri = cursor
                .getString(ContactUtil.INDEX_PHOTO_URI);
        final String destination = cursor
                .getString(ContactUtil.INDEX_PHONE_EMAIL);
        final int destinationType = cursor
                .getInt(ContactUtil.INDEX_PHONE_EMAIL_TYPE);
        final String destinationLabel = cursor
        ...
        return createRecipientEntry(displayName,
                DisplayNameSources.STRUCTURED_NAME, destination,
                destinationType, destinationLabel, contactId, lookupKey,
                dataId, photoThumbnailUri, isFirstLevel);
    }
    ...
    public static boolean isWorkProfileSupported() {
        final PackageManager pm = Factory.get().getApplicationContext()
                .getPackageManager();
    ...
    private static Uri getEmailContentLookupUri() {
        if (isWorkProfileSupported() && OsUtil.isAtLeastM()) {
            // TODO: use Email.ENTERPRISE_CONTENT_LOOKUP_URI, which will be
            // available in M SDK API
            return Uri
                    .parse("content://com.android.contacts/data/emails/lookup_enterprise");
        }
     ...
     * Returns if managed profile is supported.
     */
    public static boolean isSprdWorkProfileSupported(PackageManager pm) {
        /*final PackageManager pm = Factory.get().getApplicationContext()
                .getPackageManager();*/
        return pm.hasSystemFeature(PackageManager.FEATURE_MANAGED_USERS);
    }
     ...
     * Returns if a given contact id belongs to managed profile.
     */
    public static boolean isSprdEnterpriseContactId(final long contactId,boolean isProfileSupported) {
