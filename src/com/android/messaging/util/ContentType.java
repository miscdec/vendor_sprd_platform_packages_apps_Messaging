...
public final class ContentType {
    ...
    public static String AMR_EXTENSION = "amr";
    ...
    /* Modify by SPRD for Bug:649507 Start */
       public static final String TEXT_OTHER        = "text/*";
   /* Modify by SPRD for Bug:649507 end */
    ...
    public static boolean isMediaType(final String contentType) {
        return !isEmptyContentType(contentType)
     ...
     */
    public static boolean isUnSupportedVideo(String contentType) { // Modify for 584793
    ...
    public static boolean isUnSupportedImage(String contentType) {
        if (TextUtils.isEmpty(contentType)) {
            return false;
        }
    ...
    public static boolean isUnSupportedAudio(String contentType) { // Modify for 584793
        if (TextUtils.isEmpty(contentType)) {
            return false;
        }
    ...
    public static boolean isUnSupportedOther(String contentType) { // Modify for 584793
        if (TextUtils.isEmpty(contentType)) {
            return false;
        }
     ...
     */
    public static boolean isUnSupportedContentType(String contentType) {
        // And more or remove them, if possible.
        return !isEmptyContentType(contentType)
                && (isUnSupportedImage(contentType)
                || isUnSupportedAudio(contentType)
    ...
    private static boolean isEmptyContentType(String contentType) {
        return TextUtils.isEmpty(contentType);
    }

    /**
     * Extract content type from pdu part, whose content type is not media type, such as 'application/oct-stream'.
     *
     * @param part PduPart
     * @return String content type.
     */
    public static String tryExtractContentTypeFromPduPart(PduPart part) {
        if (part == null) {
            return "";
        }
        String contentType = "";
        try {
            Integer charset = part.getCharset();
            // cl
            String fileName = pduDataToString(part.getContentLocation(), charset);
            if (TextUtils.isEmpty(fileName)) {
                // name
                fileName = pduDataToString(part.getName(), charset);
            }
            if (!TextUtils.isEmpty(fileName)
                    // exclude smil.xml
                    && !"smil.xml".equals(fileName)) {
                // Extract content type from file suffix name.
                ...
                if (dotIdx > -1 && dotIdx < fileName.length() - 1) {
                    ...
                    if (!TextUtils.isEmpty(suffix)
                            // exclude dat suffix
                            && !"dat".equals(suffix)) {
                        contentType = MimeTypeMap.getSingleton().getMimeTypeFromExtension(suffix);
                    }
                }

                // Extract content type from AOSP encoding.
                String tempType = ""; // For log.
                if (TextUtils.isEmpty(contentType)) {
                    if (fileName.startsWith("image")) {
                        // FIXME: Cannot extract gif image.
                        tempType = IMAGE_JPEG;
                    } else if (fileName.startsWith("video")) {
                        tempType = VIDEO_MP4;
                    } else if (fileName.startsWith("recording")) {
                        tempType = AUDIO_AMR;
                    } else if (fileName.startsWith("calendar")) {
                        tempType = TEXT_VCALENDAR;
                    } else if (fileName.startsWith("contact")) {
                        tempType = TEXT_VCARD;
                    }
                    contentType = tempType;
                }
                SprdLogUtil.dump("tryExtractContentTypeFromPduPart...fileName", fileName,
                        "tempType", tempType,
    ...
    public static String pduDataToString(final byte[] bytes, Integer mibEnumValue) {
        ...
        try {
            ...
            if (mibEnumValue > 0) {
                try {
                    charsetName = CharacterSets.getMimeName(mibEnumValue);
                } catch (Exception e) {
                    charsetName = "";
                }
            }
            if (TextUtils.isEmpty(charsetName)) {
                charsetName = CharacterSets.MIMENAME_ISO_8859_1;
            }
