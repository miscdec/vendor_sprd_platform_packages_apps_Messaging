...
public class AudioAttachmentView extends LinearLayout implements
    ...
    private final static int UIMSG = 100;
    private Handler mUiHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case UIMSG:
                    try {
                        Log.d(TAG, " stop player " + mMediaPlayer + " " + mMediaPlayer.isPlaying());
                        if (mMediaPlayer.isPlaying()) {
                            resetToZeroState();
                        }
                    } catch (Exception ex) {
                        Log.d(TAG, " stop " + ex.fillInStackTrace());
                    }
                    break;
            }
        }
        ...
        mPlayPauseButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(final View v) {
                ...
                if (mMediaPlayer != null && mPrepared) {
                    if (mMediaPlayer.isPlaying()) {
                        if (mUiHandler.hasMessages(UIMSG))
                            mUiHandler.removeMessages(UIMSG);
                        mMediaPlayer.pause();
                        mChronometer.pause();
                        mProgressBar.pause();
                        isManPaused = true;
     ...
     *
     * @param incoming indicates whether the attachment view is to be styled as a
...
//        }
    ...
    private void playAudio() {
        ...
        mMediaPlayer.start();
        Log.d(TAG, " start player " + mAudioDuration);
        try {
            if (mAudioDuration > 0) {
...
//                    }
//                }, mAudioDuration + 200);
                mUiHandler.sendEmptyMessageDelayed(UIMSG, mAudioDuration + 200);
            }
        } catch (Exception ex) {
            Log.d(TAG, "setup audio duration ex");
        }
    }
    ...
    private void onAudioReplayError(final int what, final int extra,
        ...
        switch (what){
            case MediaPlayer.MEDIA_ERROR_UNKNOWN:
                UiUtils.showToastAtBottom(R.string.audio_format_unknow);
                return;
        }
        // SPREAD: FIX FOR BUG 502893  START
        if (!isLooping) {
                        ...
                        .setOnCompletionListener(new OnCompletionListener() {
                            ...
                            public void onCompletion(final MediaPlayer mp) {
                                try {
                                    ...
                                    Log.d(TAG, " onCompletion " + ex.fillInStackTrace());
                                    return;
                                }
                                if (mUiHandler.hasMessages(UIMSG))
                                    mUiHandler.removeMessages(UIMSG);
    ...
    private void updatePlayPauseButtonState() {
        ...
        // add for bug 615207 --begin
        if (!playing && mUiHandler.hasMessages(UIMSG)) {
            mUiHandler.removeMessages(UIMSG);
        }
        // add for bug 615207 --end
    ...
    private boolean isAudioFileExists(Context context, Uri uri) {
        ...
        try {
            ...
            if (cursor != null && cursor.getCount() > 0 && cursor.moveToFirst()) {
                ...
                String fileString = cursor.getString(nameIndex);
                Long size = cursor.getLong(sizeIndex);
                if (fileString == null || "".equals(fileString.trim()) || size <= 0) {
                    return false;
                }
        ...
        }/*Add by SPRD for Bug:543263 2016.03.29 Start*/ catch (SecurityException se) {
            Log.d(TAG, "AudioAttachmentView : has no permission and the exception is :" + se);
        }/*Add by SPRD for Bug:543263 2016.03.29 Start*/ finally {
            if (cursor != null) {
                cursor.close();
                cursor = null;
            }
