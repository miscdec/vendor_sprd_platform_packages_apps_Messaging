...
public class ContactListItemView extends LinearLayout implements OnClickListener {
    public interface HostInterface {
        ...
        PhoneUtils getPhoneUtils();
        boolean getIsProfileSupported();
    }
    ...
    private static final String iconSimUris[] = {
            "android.resource://com.android.messaging/SIM1"  ,
            "android.resource://com.android.messaging/SIM2"};
    ...
    public ContactListItemView(final Context context, final AttributeSet attrs) {
		...
		LogUtil.d("ContactListItemView", "ContactListItemView");
    }
    ...
    public void bind(final Cursor cursor, final HostInterface hostInterface,
        ...
        mData.bind(cursor, headerLabel,hostInterface.getIsProfileSupported());
    ...
    private void updateViewAppearance() {
        CharSequence nDestination=mData.getDestination();
        //mContactNameTextView.setText(mData.getDisplayName());
        /*bug 1065720, begin*/
        CharSequence nDisplayName=mData.getDisplayName();
        if(View.LAYOUT_DIRECTION_RTL == TextUtils.getLayoutDirectionFromLocale(Locale.getDefault())
                && nDestination != null){
            final String destinationStr = nDestination.toString().replace(" ","");
            final String displayNameStr = nDisplayName.toString().replace(" ","");
            final boolean isNumber = destinationStr.matches("^[a-zA-Z0-9\\W]+$");
            final boolean isNumber2 = displayNameStr.matches("^[a-zA-Z0-9\\W]+$");
            //final boolean isNumber= destinationStr.matches("^[0-9]*$");

            if(isNumber){
                mContactDetailsTextView.setTextDirection(View.TEXT_DIRECTION_LTR);
            }
            if(isNumber2){
                mContactNameTextView.setTextDirection(View.TEXT_DIRECTION_LTR);
            }
            mContactNameTextView.setText(nDisplayName);
            mContactDetailsTextView.setText(nDestination);
        }else{
            mContactNameTextView.setText(nDisplayName);
            /*bug 1065720, end*/
            mContactDetailsTextView.setText(nDestination);
        ...
        if(mData.getMimetype()!=null && mData.getMimetype().indexOf("email") != -1){
            LogUtil.d("ContactListItemView", "Email.getTypeLabel");
            mContactDetailTypeTextView.setText(Email.getTypeLabel(getResources(),
        ...
        LogUtil.d("ContactListItemView", "updateViewAppearance mData.getMimetype():"+mData.getMimetype()+" getDestinationType:"+mData.getDestinationType()+" getDestinationLabel:"+mData.getDestinationLabel());
        final RecipientEntry recipientEntry = mData.getRecipientEntry();
        final String destinationString = String.valueOf(nDestination);
        ...
        if (mData.getIsSimpleContactItem()) {
            ...
            final Uri avatarUri = AvatarUriUtil.createAvatarUri(
                    ParticipantData.getSprdFromRecipientEntry(recipientEntry,mHostInterface.getPhoneUtils()));
