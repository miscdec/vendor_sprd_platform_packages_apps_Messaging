...
public class AddContactsConfirmationDialog implements DialogInterface.OnClickListener {
    ...
    public void show() {
        ...
        final Button cancelButton = mAlertDialog.getButton(DialogInterface.BUTTON_NEGATIVE);
        if (cancelButton != null) {
            cancelButton.setTextColor(resources.getColor(R.color.contact_picker_button_text_color));
        }
        final Button addButton = mAlertDialog.getButton(DialogInterface.BUTTON_POSITIVE);
        if (addButton != null) {
    ...
    private View createBodyView() {
        ...
        final BidiFormatter bidiFormatter = BidiFormatter.getInstance();//by 1213341
        final String numberText = bidiFormatter.unicodeWrap(mNormalizedDestination, TextDirectionHeuristics.LTR);
        ...
        final String vocalizedDisplayName = AccessibilityUtil.getVocalizedPhoneNumber(
                mContext.getResources(), numberText);
