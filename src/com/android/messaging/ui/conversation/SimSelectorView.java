...
public class SimSelectorView extends FrameLayout implements SimSelectorItemView.HostInterface {
    ...
    private class SimSelectorAdapter extends ArrayAdapter<SubscriptionListEntry> {
        ...
        public void bindData(final List<SubscriptionListEntry> newList) {
            clear();
            /* Add by SPRD for Bug:551568 2016.04.18 Start */
            if(newList != null && newList.size() > 1) {
                Collections.sort(newList, new Comparator<SubscriptionListEntry>() {
                    @Override
                    public int compare(SubscriptionListEntry lhs, SubscriptionListEntry rhs) {
                        int lSlotId = 0;
                        int rSlotId = 0;
                        try {
                            lSlotId = Integer.parseInt(lhs.iconUri.getQueryParameter(PARAM_IDENTIFIER));
                            rSlotId = Integer.parseInt(rhs.iconUri.getQueryParameter(PARAM_IDENTIFIER));
                        } catch (Exception e) {
                        }
                        return lSlotId >= rSlotId ? 1 : -1;
                    }
                });
            }
            /* Add by SPRD for Bug:551568 2016.04.18 End */
            addAll(newList);
