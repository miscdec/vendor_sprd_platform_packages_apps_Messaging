...
public class MessageDetailsDialog {
    ...
    private static String getSmsMessageDetails(final ConversationMessageData data,
        ...
        if (!TextUtils.isEmpty(rawSender)) {
            ...
            final BidiFormatter bidiFormatter = BidiFormatter.getInstance();//by 1180825, use LTR
            details.append(bidiFormatter.unicodeWrap(rawSender, TextDirectionHeuristics.LTR));
        ...
        if (!TextUtils.isEmpty(rawRecipients)) {
            ...
            final BidiFormatter bidiFormatter = BidiFormatter.getInstance();//by 1180825, use LTR
            details.append(bidiFormatter.unicodeWrap(rawRecipients, TextDirectionHeuristics.LTR));
        }
    ...
    private static String getMmsMessageDetails(Context context, final ConversationMessageData data,
        ...
        details.append('\n');
        details.append(res.getString(R.string.from_label));
        final BidiFormatter bidiFormatter = BidiFormatter.getInstance();//bug 1206036
        if (!TextUtils.isEmpty(rawSender)) {
            details.append(bidiFormatter.unicodeWrap(rawSender, TextDirectionHeuristics.LTR));
        } else {
            details.append(res.getString(R.string.hidden_sender_address));
        }
        ...
        if (data.getSmsMessageSize() > 0) {
            ...
            details.append(Formatter.formatFileSize(context, data.getSmsMessageSize()).toUpperCase());
        }
