...
public class ConversationActivityUiState implements Parcelable, Cloneable {
    ...
    //add for jordan
        ...
        void setContactPickerNeedLoadDataFromDb(final boolean loadDataFromDb);  //Bug 1004355
    }
    ...
    private String mOldConversationId = null;
    /*Add by SPRD for bug581044  2016.07.31 End*/
    private MessageData mMessageData = null; //Bug931261
    ...
    //add for jordan
    private boolean smilFlag = false;
    public void setSmilFlag(boolean flag){
    	smilFlag = flag;
    	System.out.println("jordan, smilFlag = " + smilFlag);
    }
    //add for jordan
    /**
    ...
    public String getOldConversationId() {
        return mOldConversationId;
    }
    /*Add by SPRD for bug581044  2016.07.31 End*/

    //Bug931261 begin
    public void setMessageData(final MessageData data) {
        mMessageData = data;
    }

    public MessageData getmMessageData() {
        return mMessageData;
    }
    //Bug931261 end
    ...
    public void onGetOrCreateConversation(final String conversationId) {
        ...
        /*Add by SPRD for bug581044  2016.07.31 Start*/
        if(MmsConfig.get(ParticipantData.DEFAULT_SELF_SUB_ID).getContentEditEnabled()){
            mOldConversationId = mConversationId;
        }
        /*Add by SPRD for bug581044  2016.07.31 End*/
        mConversationId = conversationId;
    ...
    public void onStartMessageCompose() {
        ...
        if (mConversationContactUiState == STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW) {
            ...
            if(!MmsConfig.get(ParticipantData.DEFAULT_SELF_SUB_ID).getContentEditEnabled())
               ...
               performUiStateUpdate(STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW, true);
            /*Modify by SPRD for bug581044  2016.07.08 End*/
        }
        /*Add by SPRD for bug581044  2016.07.31 Start*/
        if(MmsConfig.get(ParticipantData.DEFAULT_SELF_SUB_ID).getContentEditEnabled()){
            if(mConversationContactUiState == STATE_CONVERSATION_ONLY){
                if(mConversationId != null && BugleDatabaseOperations.isEmptyConversation(mConversationId)){
                     performUiStateUpdate(STATE_CONTACT_PICKER_ONLY_INITIAL_CONTACT, true);
                }
            }
        }
        /*Add by SPRD for bug581044  2016.07.31 End*/
...
/*add for Bug 806979 start*/
    private int mNumberOfMessages = 0;
    public void ConversationMessagesUpdated(final int numberOfMessages) {
        Log.d("UiState", "onConversationMessagesUpdated numberOfMessages:" + numberOfMessages + "; mNumberOfMessages=" + mNumberOfMessages);
        if (MmsConfig.get(ParticipantData.DEFAULT_SELF_SUB_ID).getContentEditEnabled()){
            if (mNumberOfMessages == 0 && numberOfMessages > 0) {
                if (mConversationContactUiState == STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW) {
                    performUiStateUpdate(STATE_CONVERSATION_ONLY, true);
                }
            }
...
/*add for Bug 806979 end*/
    ...
    //add for jordan
        android.util.Log.d("UiState", "onAddMoreParticipants--now the state is:"+mConversationContactUiState);
        if (mConversationContactUiState == STATE_HYBRID_WITH_CONVERSATION_AND_CHIPS_VIEW || smilFlag) {
            mPendingResumeComposeMessage = true;
            performUiStateUpdate(STATE_CONTACT_PICKER_ONLY_ADD_MORE_CONTACTS, true);
        }
        //add for bug 611547 start
        // this case should not happen unless the UiState is going wrong
        else if (mConversationContactUiState == STATE_CONTACT_PICKER_ONLY_ADD_MORE_CONTACTS) {
            android.util.Log.d("UiState", "onAddMoreParticipants--new state== old state");
            notifyOnOverallUiStateChanged(mConversationContactUiState, mConversationContactUiState, true);
        }
        //add for bug 611547 end
        else {
    ...
    public void editParticipantsInDraft(){
        if(mConversationContactUiState == STATE_CONVERSATION_ONLY){
            mPendingResumeComposeMessage = true;
            mHost.setContactPickerNeedLoadDataFromDb(true);  //Bug 1004355
            performUiStateUpdate(STATE_CONTACT_PICKER_ONLY_ADD_MORE_CONTACTS, true);
        }
    }
    ...
    private void performUiStateUpdate(final int conversationContactState, final boolean animate) {
        ...
        beginUiUpdate();
        android.util.Log.d("UiState", "the oldstate is:" + mConversationContactUiState+" and the new state is:"+conversationContactState);
        //android.util.Log.d("UiState", "performUiStateUpdate ", new Exception());
        if (conversationContactState != mConversationContactUiState) {
            android.util.Log.d("UiState", "conversationContactState != mConversationContactUiState");
    ...
    public int getConversationContactUiState() {
        return mConversationContactUiState;
    }
