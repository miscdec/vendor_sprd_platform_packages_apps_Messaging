...
import java.util.HashMap;//add by wenbo
...
public class ConversationInputManager implements ConversationInput.ConversationInputBase {
    ...
    public interface ConversationInputHost extends DraftMessageSubscriptionDataProvider {
        ...
        void dimissPaused(boolean pasused);
        // spread: fixe for bug 516158 end
        void showServerMenu(boolean show);/*add for Bug 649493*/
    }
    ...
    public void showHideMediaPicker(final boolean show, final boolean animate) {
        ...
        mHost.showServerMenu(!show);/*add by sprd  for Bug 649493  */
    }
    ...
    public boolean showHideInternal(final ConversationInput target, final boolean show,
        ...
        if (target instanceof ConversationImeKeyboard && show) {//bug 1205543
            ((ConversationImeKeyboard) target).setRealShowing();
        }
        endUpdate();
        return true;
    }
            ...
            if (mMediaPicker == null) {
                ...
                mMediaPicker.setListener(new MediaPickerListener() {
                    ...
                    public void onDismissed() {
                        ...
                        mHost.showServerMenu(!mMediaInput.isOpen()); /*add by sprd  for Bug 649493 */
                        // spread: fixe for bug 516158 start
                        if(paused){
                            mHost.setOptionsMenuVisibility(!mMediaInput.isOpen());
                            mHost.dimissPaused(paused);
                            paused = false;
                        }else{
                            handleStateChange();
                        }
                    ...
                    public void onPendingAlarmAdded(final Date date){
                        mSink.onPendingAlarmAdded(date);
                    }
                    
                     @Override
                    public void onTextContactsAdded(final HashMap<String, String> contacts) {
                       ...
                       return;
                        ...
                        Log.d("wenbo","ConversationInputManager  text= " +text.toString());
                        mSink.onTextContactsAdded(text.toString());
                    }
                    @Override
                    public void StartSlideshow() {
                        /* Add by SPRD for bug 589139 2016.08.16 Start */
                        // Do not show MediaPicker after returning from SMIL
                        mShowing = false;
                        /* Add by SPRD for bug 589139 2016.08.16 End */
                        mSink.StartSlideshow();
                    }
                    ...
                    // spread: fixe for bug 516158 start
                    @Override
                    public void onPaused(boolean pause) {
                        paused = pause;
                    }
                    // spread: fixe for bug 516158 end
                });
    ...
    private class ConversationImeKeyboard extends ConversationInput {
        private boolean realShowing;//bug 1205543

        public ConversationImeKeyboard(ConversationInputBase baseHost, final boolean isShowing) {
            super(baseHost, isShowing);
            realShowing = isShowing;
        }

        @Override
        public boolean show(boolean animate) {
            realShowing = ImeUtil.get().showImeKeyboard(mContext, mSink.getComposeEditText());
            return true;
        }
        ...
        private void setRealShowing() {//bug 1205543
            mShowing = realShowing;
        }
    }
    //bug 955039 begin
    public void HideMediaPickerFirst(final boolean animate) {
        showHideInternal(mMediaInput, false, animate);
    }
    //bug 955039 end
}
