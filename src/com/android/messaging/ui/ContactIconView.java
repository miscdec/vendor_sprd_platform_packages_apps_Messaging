...
public class ContactIconView extends AsyncImageView {
    ...
    //839669 begin
    public boolean onTouchEventWithoutColorFilter(MotionEvent event) {
        return super.onTouchEvent(event);
    }
    //839669 end
    /**
    ...
    public void setImageResourceUri(final Uri uri, final long contactId,
        ...
        if (uri == null) {
            ...
            if(MmsConfig.getCmccSdkEnabled() && uri.toString().startsWith("http")){//temp, cmcc try first.
                setCircleFlag(true);
                Glide.with(getContext()).load(uri)
                        .placeholder(R.drawable.ic_default_person)
                        .error(R.drawable.ic_default_person_error)
                        .into(this);
            }else{
    ...
    public void updateaColorFilter(MotionEvent event) {//bug 1205983
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN) {
            setColorFilter(mColorPressedId);
        } else {
            clearColorFilter();
