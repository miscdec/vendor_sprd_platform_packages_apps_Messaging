...
//        searchStringParameter = searchStringParameter.trim();
        ...
        mQueryHandler = new AsyncQueryHandler(mContentResolver) {
            protected void onQueryComplete(int token, Object cookie, Cursor c) {
                ...
                Bundle bundle = intentTest.getExtras();
                //bug 842523 start
                if(bundle != null){
                    ...
                    if(WhiteTestType.WHITE_BOX_TEST_VALUE.equals(testType) &&
                            WhiteTestType.TEST_SEARCH.equals(testTask)){
                        int resultCode = -1;
                        if (c == null){
                            resultCode = 0;
                        }else{
                            resultCode =  c.getCount();
                        }
                        WhiteTestType.getInstance().sendTestBroadcase(testType,resultCode);
                        finish();
                    }
...
//                        Contact contact = address != null ? Contact.get(address, false) : null;
                  ...
                  //      System.out.println("jordan, recNameStr = [" + "******" + "], addressStr = [" + "******" + "]");
                        title.setText(TextUtils.isEmpty(recNameStr) ? addressStr : recNameStr);
                        /* SPRD: Modify this for search function. @{ */
...
//                                startActivity(onClickIntent);
                                UIIntents.get().launchConversationActivity(context, String.valueOf(convId), null, null, false);
                            }
                        });
                    }
        ...
        //Bug 880287
        Uri.Builder b = Uri.parse("content://com.android.messaging.datamodel.MessagingContentProvider/search").buildUpon();
        b = b.appendQueryParameter("pattern",searchString);
        mUri = b.build();
/*
