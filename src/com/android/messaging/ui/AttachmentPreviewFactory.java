...
public class AttachmentPreviewFactory {
     ...
     * For smil view
     */
    public static final int TYPE_SMIL = 4;
    ...
    public static View createAttachmentPreview(final LayoutInflater layoutInflater,
        ...
        if (TYPE_SMIL == viewType) {
            attachmentView = createSmilPreview(layoutInflater, attachmentData, parent, viewType);
        } else {
        ...
        /* Add by SPRD for bug 578200 Start */
            } else if (ContentType.APP_SMIL.equals(contentType)) {
                attachmentView = createSmilPreview(layoutInflater, attachmentData, parent, viewType);
        /* Add by SPRD for bug 578200 End */
            } else {
    ...
    private static View createVCardPreview(final LayoutInflater layoutInflater,
        ...
        vcardPreview.disableContactIconClickAction();  //Bug 1180725
        return view;
    }
    ...
    /* Add by SPRD for bug 578200 Start */
    private static View createSmilPreview(final LayoutInflater layoutInflater,
                                          final MessagePartData attachmentData, final ViewGroup parent,
                                          final int viewType) {
        int layoutId = R.layout.attachment_smil;
        final String smilText = attachmentData.getText();
        attachmentData.setText("");
        final View view = layoutInflater.inflate(layoutId, parent, false /* attachToRoot */);
        AsyncImageView smilThumbnail = (AsyncImageView) view.findViewById(R.id.smil_thumbnail);
        TextView smilTextView = (TextView) view.findViewById(R.id.smil_text);
        if (!TextUtils.isEmpty(smilText)) {
            smilTextView.setText(smilText);
        }
        final String contentType = attachmentData.getContentType();
        if (ContentType.isImageType(contentType)) {
            final Resources resources = layoutInflater.getContext().getResources();
            final int maxWidthAndHeight = resources.getDimensionPixelSize(
                    R.dimen.smil_attachment_preview_size);
            smilThumbnail.setImageResourceId(getImageRequestDescriptorForAttachment(attachmentData,
                    maxWidthAndHeight, maxWidthAndHeight));
        } else if (ContentType.isAudioType(contentType)) {
            smilThumbnail.setImageResource(R.drawable.ic_preview_play);
        } else if (ContentType.isVideoType(contentType)) {
            MessagePartVideoThumbnailRequestDescriptor messagePartVideoThumbnailRequestDescriptor = new MessagePartVideoThumbnailRequestDescriptor(
                    attachmentData);
            smilThumbnail.setImageResourceId(messagePartVideoThumbnailRequestDescriptor);
        } else {
            smilThumbnail.setImageResource(R.drawable.ic_attach_slideshow);
        }
        return view;
    }
    /* Add by SPRD for bug 578200 End */
