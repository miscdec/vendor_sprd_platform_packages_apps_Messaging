...
public class ConversationListItemView extends FrameLayout implements OnClickListener,
    ...
    private void setConversationName() {
        ...
        final String conversationName = mData.getName() == null ? "" : mData.getName();
    ...
    public static String nFormattedTimestamp;
    ...
    public static String buildContentDescription(final Resources resources,
        ...
        if(nFormattedTimestamp==null){
            nFormattedTimestamp=data.getFormattedTimestamp();
        }
     ...
     * entry.
     * @param mAdapter callback, for smart message base CMCC
     */
    public void bind(final Cursor cursor, final HostInterface hostInterface, ConversationListAdapter mAdapter) {
        // Update our UI model
        ...
        setConversationName();
        setSubject();
        nFormattedTimestamp = mData.getFormattedTimestamp();
      ...
      //  final boolean isDefaultSmsApp = PhoneUtils.getDefault().isDefaultSmsApp();
        ...
        if (mData.getIsFailedStatus() && mHostInterface.isDefaultSmsApp()) {
            ...
            // add for bug 532412 begin
            if (typefaceStyle == Typeface.ITALIC) {
                mTimestampTextView.append(" ");
            }
            // add for bug 532412 end
         } else {
            mTimestampTextView.setTextColor(mListItemReadColor);
            mTimestampTextView.setTypeface(mListItemReadTypeface, typefaceStyle);
            final String formattedTimestamp = nFormattedTimestamp;
        ...
        // fix for bug 547780 begin
        if (!isSelected) {
            mHostInterface.updateSelectedAllOrNoneIconState();
        }
        // fix for bug 547780 end
        Uri iconUri = null;
        ...
        if (isSelected) {
            ...
            if (mData.getIsFailedStatus() && !mData.getIsGroup() && mHostInterface.isDefaultSmsApp()) {
        ...
        // fix for bug 757892  begin
        if(mAdapter != null && mAdapter.isScrolling()){
            mContactIconView.setImageResource(R.drawable.ic_default_person);
            mContactIconView.setDefaultIconFlag(true);//1109962
        }else{
        ...
        if (previewImageUri != null && previewImageUri.toString().contains("content://com.android.email.attachmentprovider") && !OsUtil.hasEmailPermission()) {
            mHostInterface.needReadEmailAttachmentPermission();
        }
    ...
    public final void setAnimating(final boolean animating) {
        ...
        if (mAnimatingCount == 0) {
            ...
            setShortAndLongClickable(true);
            mSwipeableContainer.setPressed(false); //bug 1218589
