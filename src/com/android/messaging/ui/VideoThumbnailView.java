...
public class VideoThumbnailView extends FrameLayout {
    ...
    protected void onAnimationStart() {
        ...
        mAnimating = true;
        if (mPlayButton != null) {
            mPBBG = mPlayButton.getBackground();
            mPlayButton.setBackgroundDrawable(null);
        }
    }
    ...
    protected void onAnimationEnd() {
        ...
        mAnimating = false;
        if (mPlayButton != null) {
            mPlayButton.setBackgroundDrawable(mPBBG);
            mPBBG = null;
        }
        trySwitchToVideo();
    }
    ...
    public void setSource(final MessagePartData part, final boolean incomingMessage) {
        mPart = part;
        Log.d(TAG, " setSource mPart is  " + mPart);
        if (part == null) {
            clearSource();
        } else {
            ...
            if (shouldUseGenericVideoIcon(incomingMessage)) {
                ...
                MessagePartVideoThumbnailRequestDescriptor messagePartVideoThumbnailRequestDescriptor = new MessagePartVideoThumbnailRequestDescriptor(part);
                Log.d(TAG, " setSource path is  " + part.getDrmDataPath());
                if (part.isDrmType()) {
                    messagePartVideoThumbnailRequestDescriptor.setDrmContentType(part.getContentType());
                    messagePartVideoThumbnailRequestDescriptor.setDrmType(true);
                    messagePartVideoThumbnailRequestDescriptor.setDrmDataPath(part.getDrmDataPath());
                    //bug808139
                    mPlayButton.setVisibility(View.INVISIBLE);
                    mThumbnailImage.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(final View view) {
                            if (mVideoSource == null) {
                                return;
                            ...
                            String contentType = mPart.getContentType();
                            boolean isDrm = ContentType.isDrmType(contentType);
                            Log.d(TAG, " mThumbnailImage content type is " + contentType + " is drm " + isDrm);
                            if (isDrm) {
                                //String path = mPart.getDrmDataPath();
                                Log.d(TAG, " mThumbnailImage content type is " + contentType + " path " + mPart.getDrmDataPath());
                                if (mPart.getDrmFileRightsStatus() == false) {
                                    Factory.get().getUIIntents().launchDrmRightRequestActivity(mContext, mPart);
                                    return;
                                }
                ...
                mThumbnailImage.setImageResourceId(messagePartVideoThumbnailRequestDescriptor);
                if (mVideoView != null) {
    ...
    public void setSource(final Uri videoSource, final boolean incomingMessage) {
        Log.d(TAG, " setSource uri is  " + videoSource);
        if (videoSource == null) {
            clearSource();
        } else {
            ...
            if (shouldUseGenericVideoIcon(incomingMessage)) {
                ...
                MessagePartVideoThumbnailRequestDescriptor messagePartVideoThumbnailRequestDescriptor = new MessagePartVideoThumbnailRequestDescriptor(videoSource);
                try {
                    messagePartVideoThumbnailRequestDescriptor.setDrmDataPath(MessagingDrmSession.get().getPath(videoSource));
                } catch (Exception ex) {
                    Log.d(TAG, " setSource uri ex");
                }
                mThumbnailImage.setImageResourceId(messagePartVideoThumbnailRequestDescriptor);
                if (mVideoView != null) {
    ...
    private void playVideo() {
        if (mMode == MODE_PLAYABLE_VIDEO) {
            ...
            if (mPart != null) {
                isDrm = mPart.isDrmType();
                dataPath = mPart.getDrmDataPath();
            }
            if (isDrm) {
                UIIntents.get().launchFullScreenVideoViewerForDrm(getContext(), dataPath);
            } else {
                UIIntents.get().launchFullScreenVideoViewer(getContext(), mVideoSource);
            }
        }
