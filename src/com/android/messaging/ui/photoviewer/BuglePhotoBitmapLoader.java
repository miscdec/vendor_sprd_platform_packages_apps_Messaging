...
public class BuglePhotoBitmapLoader extends AsyncTaskLoader<BitmapResult>
    ...
    private static final String TAG = "BuglePhotoBitmapLoader";

    private final OnLoadListener mListener;

    private static AtomicLong sDrmKeyEnd = new AtomicLong(1);

    public BuglePhotoBitmapLoader(Context context, String photoUri, OnLoadListener listener) {
        super(context);
        mPhotoUri = photoUri;
        mListener = listener;
    }
    ...
    public BitmapResult loadInBackground() {
        ...
        if (context != null && mPhotoUri != null) {
            ...
            Uri dataUri = Uri.parse(mPhotoUri);
            try {
                ...
                if (dataPath != null && MessagingDrmSession.get().drmCanHandle(dataPath, null)) {
                    ...
                    descriptor.setupDrm(true, ContentType.APP_DRM_CONTENT, dataPath);
                    descriptor.setDrmPreviewInPhotoViewActivity(true);
                    descriptor.reportDrmEndKey(sDrmKeyEnd.getAndIncrement());
                    DecryptHandle decryptHandle = MessagingDrmHelper.setDrmPlayStatusStart(dataPath);
                    descriptor.setDrmDecryptHandle(decryptHandle);
                    if (mListener != null) mListener.onLoading(decryptHandle);
                }
            } catch (Exception ex) {
                Log.d(TAG, " drm ex " + ex);
            }
