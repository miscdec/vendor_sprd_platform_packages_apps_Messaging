                ...
                try{
                    ...
                    if (path!=null&&MessagingDrmSession.get().drmCanHandle(path, null)){
                        ...
                        if (!ContentType.isImageType(drmOrigContentType)) {
                           ...
                           return;
                        }else if (!isDrmImageSupport(drmOrigContentType)){
                            UiUtils.showToastAtBottom(R.string.orgdrm_no_share);
                            return;
                        }
...
//                        }
                ...
                long  size=0;
                try {
                    size=MmsUtils.getMediaFileSize(documentUri);
                    isGif = ImageUtils.isGif(pendingItem.getContentType(), documentUri);
                }catch (Exception e){
                    LogUtil.e(TAG, "onPostExecute...fail:" + e, e);
                }
                if(isGif){
                    if(size< MmsConfig.getMaxMaxMessageSize()){
                        MmsUtils.setImageWidthHeight(pendingItem,documentUri);
                    }
                }else{
                ...
                //sprd 623241 start
                Log.d(TAG,"Width:"+pendingItem.getWidth()+" height:"+pendingItem.getHeight()+" documentUri:"+documentUri);
                //sprd 623241 end
