...
class GalleryMediaChooser extends MediaChooser implements
        GalleryGridView.GalleryGridViewListener, MediaPickerDataListener, MediaPickerPanel.IAnimationEvent {
...
//    }
    public int[] getIconResource() {
        return new int[] {R.drawable.ic_image_light, R.drawable.ic_image_dark};
    }
    ...
    protected void setSelected(final boolean selected) {
        ...
        if (selected && !OsUtil.hasStoragePermission()) {
            mMediaPicker.requestPermissionsFromMediaPicker(
    ...
    @Override
    public void onGalleryMediaEvent(boolean allowBinding) {//by 1186186
        if (null == mAdapter) {
            return;
        }
        mAdapter.setBindingDataFlag(allowBinding);
        if(allowBinding){
