...
public class VideoAttachPicker {
    ...
    public static String getVideoContentType(final ContentResolver cr,
        ...
        Cursor cursor = null;
        /*SPRD for bug 649283 add start 20170303.*/
        Log.d(TAG, "uri = " + uri);
        if (uri == null) {
            Log.d(TAG, "uri == null");
        ...
        if (null == uriAuthority) {
            Log.d(TAG, "uriAuthority is null. uri=" + uri);
            return ContentType.VIDEO_UNSPECIFIED;
        }
        /*SPRD for bug 649283 add end.*/
        if (UriUtil.isMediaStoreUri(uri)) {
            ...
            } catch (SecurityException e){//add for 1376537, monkey test
                e.printStackTrace();
            } finally {
                if (cursor != null) {
                    cursor.close();
    ...
    private void prepareVideoAttachForAttachment(final Uri VideoAttachUri) {
        ...
        new SafeAsyncTask<Void, Void, String>() {
            ...
            protected String doInBackgroundTimed(final Void... params) {
                return getVideoContentType(Factory.get()
