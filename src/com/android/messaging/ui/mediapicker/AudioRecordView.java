...
public class AudioRecordView extends FrameLayout implements
    ...
    private void updateRecordButtonAppearance() {
        /*Add By SPRD for Bug:586616 {@*/
        final Resources resources = Factory.get().getApplicationContext().getResources();
        final int getColorFromConverstionTheme = resources.getColor(R.color.primary_color);
        /*@}*/
        ...
        if (isRecording()) {
            ...
            /*Add By SPRD for Bug:586616 {@*/
            if (mThemeColor != getColorFromConverstionTheme) {
                mThemeColor = getColorFromConverstionTheme;
            }
            /*@}*/
    ...
    boolean onRecordButtonTouchDown() {
        if (!mMediaRecorder.isRecording() && mCurrentMode == MODE_IDLE) {
            /* Add by SPRD for bug 554677 2016/05/04 Start */
            if(SystemAdapter.getInstance().isAudioRecording()) {//mAudioManager.isAudioRecording()) {
                UiUtils.showToastAtBottom(R.string.audio_record_retry);
                return false;
            }
            /* Add by SPRD for bug 554677 2016/05/04 End */
            /* Add by SPRD for bug 587760 {@ */
            if (!OsUtil.hasRecordAudioPermission()) {
                UiUtils.showToastAtBottom(R.string.no_record_audio_permission);
                return false;
            }
            /*@}*/
            ...
            playAudioStartSound(new OnCompletionListener() {
                ...
                public void onCompletion() {
                    /* Add by SPRD for Bug 553281 Start */
                    ...
                    /* Add by SPRD for Bug 553281 end */
                    ...
                    if(GlobleUtil.isSmilAttament){ 
                        final int maxSize = 300 * 1024;
                        /*Modify by SPRD for Bug:553281 Start*/
                        if (mCurrentMode == MODE_STARTING &&
                                mMediaRecorder.startRecording(AudioRecordView.this,
                                        AudioRecordView.this, maxSize - (int)attachmentSize)) {
                        /*Modify by SPRD for Bug:553281 end*/
        ...
        if (outputUri != null) {
            ...
            final MediaPickerMessagePartData audioItem =
                    new MediaPickerMessagePartData(startRect,
                            ContentType.AUDIO_AMR, outputUri, 0, 0);
            ...
            try {
                            ...
                            public void save(final Uri uri) {
                                new SafeAsyncTask<Uri, Void, Void>(60 * 1000, true) {
                                    ...
                                    protected Void doInBackgroundTimed(Uri... params) {
                                        try {
                                            Uri persisituri = UriUtil.persistContent(uri, new File(UriUtil.getValidSaveAttachmentPath()), ContentType.AUDIO_AMR);
                                            UriUtil.scannerSaveAttachment(Factory.get().getApplicationContext(), persisituri);
                                        } catch (Exception ex) {
                ...
                Log.d("AudioRecordView", " showSaveAttachmentInfo ", ex.fillInStackTrace());
            }
        }
        playAudioEndSound();
        mAudioManager.abandonAudioFocus(mOnAudioFocusChangeListener);
    ...
    private OnAudioFocusChangeListener mOnAudioFocusChangeListener = new OnAudioFocusChangeListener() {
        ...
        public void onAudioFocusChange(int focusChange) {
            Log.d("AudioRecordView", "========zhongjihao=====focusChange: "
                    + focusChange + "  mCurrentMode: " + mCurrentMode);
            switch (focusChange) {
