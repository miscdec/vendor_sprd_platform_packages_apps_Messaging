...
class MmsVideoRecorder extends MediaRecorder {
    // modify for sprd bug #493807,change from 85% to 97%
    private static final float VIDEO_OVERSHOOT_SLOP = .93F; //for Bug 609803 

    private static final int BITS_PER_BYTE = 8;

    // We think user will expect to be able to record videos at least this long
    private static final long MIN_DURATION_LIMIT_SECONDS = 12;//Bug 819197
    ...
    public MmsVideoRecorder(final Camera camera, final int cameraIndex, final int orientation,
        ...
        //Bug 1000565 begin
        if (cameraIndex == 0) { //background camera
            mCamcorderProfile =
                    CamcorderProfile.get(cameraIndex, CamcorderProfile.QUALITY_480P);//Bug 819197
        } else {  //forwardground camera cameraIndex = 1
            mCamcorderProfile =
                    CamcorderProfile.get(cameraIndex, CamcorderProfile.QUALITY_QCIF);//Bug 819197
        }
        //Bug 1000565 end
        ...
        // modify for sprd bug #493807
   //     final long sizeLimit = (long) (maxMessageSize - MIN(maxMessageSize
    //            * (1 - VIDEO_OVERSHOOT_SLOP), 9 * 1024));
    //for Bug 609803
        ...
        initSizeLimit(sizeLimit);
        /* Add by SPRD for Bug 549991 end */
    }

    private int MIN(float overShoot, int overShootValue) {
        // TODO Auto-generated method stub
        if ((int) overShoot >= overShootValue) {
    ...
    /* Add by SPRD for Bug 549991 Start */
    private static long mSizeLimit;

    private void initSizeLimit(long sizeLimit) {
      //  if(sizeLimit > mSizeLimit) {
