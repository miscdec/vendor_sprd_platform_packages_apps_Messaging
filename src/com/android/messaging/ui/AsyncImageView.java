...
public class AsyncImageView extends ImageView implements MediaResourceLoadListener<ImageResource> {
     ...
     */
    //bug725689 beign
    private boolean mDrmLockedState = false;
    //bug725689 end
    public void setImageResourceId(@Nullable final ImageRequestDescriptor descriptor) {
        ...
        //bug725689 beign
        final boolean isDrm = (descriptor == null) ? false : descriptor.getDrmType();
        final boolean drmLockedState = (descriptor == null) ? false : descriptor.getDrmLockedState();
        //bug725689 end
        if (mImageRequestBinding.isBound()) {
            if (!isDrm && TextUtils.equals(mImageRequestBinding.getData().getKey(), requestKey) && httpLogoLoaded(descriptor,requestKey)
                    && !isDefaultIcon) {//bug 1109962
                // Don't re-request the bitmap if the new request is for the same resource.
                return;
            }
            /*bug 1109962, begin*/
            resetDefaultIconFlag();
            /*bug 1109962, end*/
            //bug725689 beign
            if(isDrm && mDrmLockedState == drmLockedState){
                return;
            }
            //bug725689 end
            unbindView();
        ...
        if (!TextUtils.isEmpty(requestKey)) {
            ...
            //bug725689 begin
            mDrmLockedState = drmLockedState;
            //bug725689 end
    ...
    protected void onDraw(final Canvas canvas) {
        if (mCornerRadius > 0 || useCircle) {//add for glide
            ...
            if (mClipPathWidth != currentWidth || mClipPathHeight != currentHeight) {
                ...
                if(useCircle){//add for glide
                    mRoundedCornerClipPath.addCircle(currentWidth/2, currentHeight/2, currentWidth/2, Path.Direction.CW);
                }else{
    ...
    /*smart message, begin*/
    private boolean httpLogoLoaded(final ImageRequestDescriptor descriptor, String requestKey){
        if(requestKey != null && requestKey.startsWith("http")){
            final BindableMediaRequest<ImageResource> imageRequest =
                    descriptor.buildAsyncMediaRequest(getContext(), null);
            final ImageResource imageResource = MediaResourceManager.get().loadMediaFromCache(imageRequest);
    ...
    protected void setCircleFlag(boolean circleFlag){
        this.useCircle = circleFlag;
    }
    /*smart message, end*/
    ...
    private void resetDefaultIconFlag(){
        isDefaultIcon = false;
    }

    public void setDefaultIconFlag(boolean flag){
        this.isDefaultIcon = flag;
    }
    /*bug 1109962, end*/
}
