...
public class PeopleAndOptionsFragment extends Fragment
    ...
    public void onDestroy() {
        if (mBlockConfirmDialog != null) {
            mBlockConfirmDialog.dismiss();
            mBlockConfirmDialog = null;
        }
        super.onDestroy();
        mBinding.unbind();
    }
    ...
    public void onOptionsItemViewClicked(final PeopleOptionsItemData item,
        ...
        if (!MmsConfig.getKeepOrgSoundVibrate()) {
            onOptionsItemViewClickedSPRD(item, isChecked);
        } else {
        /* SPRD: Modify for bug 509830 begin */
            switch (item.getItemId()) {
                ...
                case PeopleOptionsItemData.SETTING_BLOCKED:
                    ...
                    final String mDestination = bidiFormatter.unicodeWrap( item.getOtherParticipant().getDisplayDestination(), TextDirectionHeuristics.LTR); //by 1229545
  ...
  private AlertDialog mBlockConfirmDialog;
    /* SPRD: Add for bug 509830 begin */
    public void onOptionsItemViewClickedSPRD(final PeopleOptionsItemData item,
            final boolean isChecked) {
        switch (item.getItemId()) {
            case PeopleOptionsItemData.SETTING_NOTIFICATION_ENABLED:
                mBinding.getData().enableConversationNotifications(mBinding, isChecked);
            ...
            case PeopleOptionsItemData.SETTING_BLOCKED_SPRD:
                if (item.getOtherParticipant().isBlocked()) {
                    mBinding.getData().setDestinationBlocked(mBinding, false);
                    break;
                }
                ...
                final Resources res = getResources();
                final Activity activity = getActivity();
                mBlockConfirmDialog =new AlertDialog.Builder(activity)
                        .setTitle(res.getString(R.string.block_confirmation_title,
                                mDestination))
                        .setMessage(res.getString(R.string.block_confirmation_message))
                        .setNegativeButton(android.R.string.cancel, null)
    ...
    private class OptionsListAdapter extends BaseAdapter {
        ...
        public View getView(final int position, final View convertView, final ViewGroup parent) {
            ...
            if (mOptionsCursor == null || mOptionsCursor.isClosed() || mOptionsCursor.getCount() == 0) {
                getActivity().finish();
                return itemView;
            }
        ...
        public View getView(final int position, final View convertView, final ViewGroup parent) {
            ...
            itemView.setListener(new PersonItemView.PersonItemViewListener() {
                ...
                public boolean onPersonLongClicked(final PersonItemData data) {
                    if (mBinding.isBound()) {
                        /*Modify by SPRD for bug:610337 {@*/
                        final CopyContactDetailDialog dialog = new CopyContactDetailDialog(
                                getContext(), data.getNormalizedDestination()/*data.getDisplayName()*/);
                        /*@}*/
                        dialog.show();
                        return true;
                    }
