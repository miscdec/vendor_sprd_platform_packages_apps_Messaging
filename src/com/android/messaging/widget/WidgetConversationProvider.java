...
public class WidgetConversationProvider extends BaseWidgetProvider {
    ...
    public static String CONVERSATION_WIDGET_CREATE = "conversation_widget_create";
    private SharedPreferences sp = Factory.get().getApplicationContext().getSharedPreferences(BugleWidgetProvider.WIDGET_STATE, Context.MODE_PRIVATE);
    /* Add by SPRD for optimization broadcast 2016.09.18 End */
    //Bug 888919 begin
    private static  long caller_count = 0;
    private static  long caller_time = 0;
    //Bug 888919 end
    ...
    public void onEnabled(Context context) {
        ...
        editor.putBoolean(CONVERSATION_WIDGET_CREATE, true);
        editor.commit();
    }
    ...
    public void onDisabled(Context context) {
        ...
        Editor editor = sp.edit();
        editor.putBoolean(CONVERSATION_WIDGET_CREATE, false);
        editor.commit();
    }
    /* Add by SPRD for optimization broadcast 2016.09.18 End */
    ...
    public static void rebuildWidget(final Context context, final int appWidgetId) {
        ...
        if (TextUtils.isEmpty(conversationId)) {
            ...
            scrollToBottom(context, appWidgetId, remoteViews, conversationId);
    ...
    private static void scrollToBottom(final Context context, final int appWidgetId, final RemoteViews remoteViews, final String conversationId) {
        LogUtil.w(TAG, "scrollToBottom  ");
       //Bug 1165789  begin
       if(null == sWorkerQueue) {
       ...
       //Bug 1165789  end
        sWorkerQueue.postDelayed(new Runnable() {
            @Override
            public void run() {
                final Uri uri = MessagingContentProvider.buildConversationMessagesUri(conversationId);
                if (uri != null) {
                    LogUtil.w(TAG, "scrollToBottom doQuery uri: " + uri.toString());
                }
                ...
                remoteViews.setScrollPosition(R.id.message_list, position);
    ...
    public static void notifyMessagesChanged(final Context context, final String conversationId) {
        ...
        caller_count ++;
        if((ct - caller_time) < 80 && caller_count >= 3){
            caller_count = 0;
            Log.e("Broadcast-too-much","notifyMessagesChanged",new Exception());
        }else if((ct - caller_time) > 80){
            Log.d("notifyMessagesChanged","clear count");
            caller_count = 0;
        }
        //Bug 888919 end
        final Intent intent = new Intent(ACTION_NOTIFY_MESSAGES_CHANGED);
        intent.setClassName("com.android.messaging","com.android.messaging.widget.WidgetConversationProvider"); //Bug966368
        intent.putExtra(UIIntents.UI_INTENT_EXTRA_CONVERSATION_ID, conversationId);
