...
import com.sprd.messaging.smart.ServiceInfoImpl;
...
public class ParticipantRefresh {
    ...
    public static final int REFRESH_SERVICEPORT_ONLY = 3;

	private static final int REFRESH_CONTACTS = 4; //Bug 894985

    private static boolean mHasNet = false;

    //UNISOC: Add for Bug#1403736
    private static final int CONVERSATION_ID_MAX = 20;
    ...
    private static final Runnable sServiceRefreshRunnable = new Runnable() {
        @Override
        public void run() {
            LogUtil.v(LogUtil.BUGLE_SMART_TAG, "Start Service Refresh");
            refreshParticipants(REFRESH_SERVICEPORT_ONLY);
        }
    };
    ...
    public static class ContactContentObserver extends ContentObserver {
        ...
        public void onChange(final boolean selfChange) {
            ...
            if (contactUpdateHandler != null) {
                contactUpdateHandler.sendEmptyMessage(REFRESH_CONTACTS);
            }
            //Bug 894985 end
        }
    ...
    public static void refreshParticipantsIfNeeded() {
        ...
        } else if(MmsConfig.isSwitchChanged()){//bug 1007571
            LogUtil.v(LogUtil.BUGLE_SMART_TAG, "isSwitchChanged current:"+MmsConfig.getCtccSmartSmsPreference());
            if(MmsConfig.getCtccSmartSmsPreference()){
                SafeAsyncTask.executeOnThreadPool(sFullRefreshRunnable);
            }else{
                SafeAsyncTask.executeOnThreadPool(sServiceRefreshRunnable);
            }
        }
    }
    ...
    public static void refreshServicePortParticipants() {
        SafeAsyncTask.executeOnThreadPool(sServiceRefreshRunnable);
    }

    public static void refreshServicePortParticipantsById(String partId) {
        SafeAsyncTask.executeOnThreadPool(new RefreshRunnableById(partId));
    }

    private static class RefreshRunnableById implements Runnable{
        private String partId;
        public RefreshRunnableById(String id){
            this.partId = id;
        }

        @Override
        public void run() {
            refreshParticipantsById(partId);
        }
    }
    ...
    public static void registerRefreshForServerPort(boolean isfullrefresh){
        LogUtil.d(LogUtil.BUGLE_SMART_TAG, "registerRefreshForServerPort, isfullrefresh: " + isfullrefresh);
        if(refreshAll){
            return;
        }
        refreshAll = isfullrefresh;
        if(null == connectivityListener){
            connectivityListener = new ConnectivityListener() {
                ...
                public void onConnectivityStateChanged(final Context context, final Intent intent) {
                    ...
                    LogUtil.d(LogUtil.BUGLE_SMART_TAG, "ConnectivityListener onConnectivityStateChanged,Connected:"+ isConnected);
                    if (isConnected) {
                        onConnected();
                    }
                }
                ...
                private void onConnected() {
                    LogUtil.d(LogUtil.BUGLE_SMART_TAG, "ConnectivityListener onConnected, refreshAll:"+ refreshAll);
                    if(refreshAll){
                        startFullRefresh();
                    }else{
                        refreshServicePortParticipants();
                    }
                    DataModel.get().getServicePortConnectivityUtil().unregister();
                }
            };
            if(!DataModel.get().getServicePortConnectivityUtil().isAlreadyRegister()){
                DataModel.get().getServicePortConnectivityUtil().register(connectivityListener);
            }
        }
    }
    /*smart message, end*/
    ...
    private static void startFullRefresh() {
        final ContactContentObserver observer = Factory.get().getContactContentObserver();
        if (observer != null) {
            observer.ContactChanged();
        }
        refreshParticipantsIfNeeded();
    }
    ...
    public static void refreshParticipantAndUpdate(final String participantId) {
        Assert.notNull(participantId);
        final DatabaseWrapper db = DataModel.get().getDatabase();
        final ArrayList<String> changedParticipants = new ArrayList<String>();
        try {
            final ParticipantData participantData = ParticipantData.getFromId(db, participantId);
            if (refreshParticipant(db, participantData)) {
                updateParticipant(db, participantData);
                changedParticipants.add(participantId);
                LogUtil.d(LogUtil.BUGLE_SMART_TAG, "refreshParticipantAndUpdate participantId:" + participantId + "; getId = "+participantData.getId());
                BugleDatabaseOperations.refreshConversationsForParticipants(changedParticipants);
            }
        } catch (final Exception exception) {
            LogUtil.e(LogUtil.BUGLE_SMART_TAG, "refreshParticipantAndUpdate: Failed to " +
                    "update participant", exception);
        }
        MessagingContentProvider.notifyParticipantsChanged(participantId);
    }
    /**
     ...
     */
     @VisibleForTesting
     synchronized static void refreshParticipants(final int refreshMode) {
        Assert.inRange(refreshMode, REFRESH_MODE_FULL, REFRESH_SERVICEPORT_ONLY);// by sprd, for smart-message
        if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
            switch (refreshMode) {
                ...
                case REFRESH_MODE_SELF_ONLY:
                    ...
                    break;
                case REFRESH_SERVICEPORT_ONLY:
                    LogUtil.v(LogUtil.BUGLE_SMART_TAG, "Start ServicePort refresh");
                    break;
            }
        }
        ...
        if(MmsConfig.supportSmartSdk()){//by smart message
            if(DataModel.get().getServicePortConnectivityUtil()!=null){
                mHasNet = DataModel.get().getServicePortConnectivityUtil().hasNet();
                if(!mHasNet){
                    registerRefreshForServerPort(true);
                }
            }
        }
        ...
        if (refreshMode == REFRESH_MODE_INCREMENTAL) {
            ...
            selectionArgs = null;
        }else if(refreshMode == REFRESH_SERVICEPORT_ONLY) {
            selection = ParticipantColumns.CONTACT_ID + "=?";
            selectionArgs = new String[] {
                    String.valueOf(ParticipantData.PARTICIPANT_CONTACT_ID_SERVICEPORT) };
        }

        final DatabaseWrapper db = DataModel.get().getDatabase();
        Cursor cursor = null;
        boolean selfUpdated = false;
        ArrayList<ParticipantData> participantDataArrayList = new ArrayList<ParticipantData>();
        try {
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    try {
                        ...
                        /*smart message, begin*/
                        if(refreshMode != REFRESH_MODE_SELF_ONLY  && !participantData.isSelf()
                                && !TextUtils.isEmpty(participantData.getSendDestination())){
                            participantDataArrayList.add(participantData);
                        }
                        /*smart message, end*/
                    } catch (final Exception exception) {
        ...
        /*smart message,begin*/
         LogUtil.d(LogUtil.BUGLE_SMART_TAG, "refreshParticipants all");
         for(ParticipantData participantData : participantDataArrayList){
             updateParticipantDataIfSmart(db.getContext(),participantData);
         }
         /*smart message,end*/
    }
    ...
    private static final Set<Integer> getExistingSubIds() {
        ...
        try {
            ...
            LogUtil.e(LogUtil.BUGLE_TAG,"getExistingSubIds exception="+exception);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    private static void refreshSelfParticipantList() {
        ...
        try {
            ...
            LogUtil.e(LogUtil.BUGLE_TAG,"refreshSelfParticipantList exception="+exception);
        } finally {
            db.endTransaction();
        }
    ...
    public static boolean refreshParticipant(final DatabaseWrapper db,
        ...
        if (participantData.isSelf()) {
            ...
            // If self profile deleted, update is true.
            // If not, also nedd call refreshFromContacts to ensure update other contacts(not self) if they are updated.
            updated = (selfChange == SELF_PHONE_NUMBER_OR_SUBSCRIPTION_CHANGED) || (selfChange == SELF_PROFILE_DELETED);
    ...
    private static final int SELF_PROFILE_DELETED = 3;
    ...
    private static int refreshFromSelfProfile(final DatabaseWrapper db,
        ...
        try {
            ...
            if (selfCursor != null && selfCursor.getCount() > 0) {
                ...
                // profile deleted.
                LogUtil.d(LogUtil.BUGLE_TAG, "lxg - refreshFromSelfProfile: orginal contactId=" + participantData.getContactId());
                if (participantData.getContactId() != ParticipantData.PARTICIPANT_CONTACT_ID_NOT_FOUND) {
    ...
    private static boolean refreshFromContacts(final DatabaseWrapper db,
        ...
        final String currentLookupKey = participantData.getLookupKey();//Add for Bug:538820
        ...
        try {
            ...
            if (matchingContactCursor == null || matchingContactCursor.getCount() == 0) {
                ...
                //bug 1003591 begin
                boolean resetParams = (currentContactId != ParticipantData.PARTICIPANT_CONTACT_ID_NOT_FOUND);
                if(MmsConfig.supportSmartSdk() && ParticipantData.PARTICIPANT_CONTACT_ID_SERVICEPORT == currentContactId){
                    resetParams = false;
                }
                //bug 1003591 end
                if (resetParams) {
        ...
        /*Add by SPRD for Bug:538820 Start */
        final boolean isLookupKeyChanged = !TextUtils.equals(matchingLookupKey, currentLookupKey);
        /*Add by SPRD for Bug:538820 End */
        if (isLookupKeyChanged || /*Add for Bug:538820 */isContactIdChanged || isDisplayNameChanged
               || isFirstNameChanged || isPhotoUrlChanged || isDestinationChanged) {
            ...
            if (MmsConfig.getFdnContactFittingEnable()) {//bug821500
                ...
                Log.d(TAG,"update from FdnContact:"+matchingDestination);
    ...
    //Bug 888919 begin
    private static void updateConversationSelfIdExt(final String conversationId,
                                                    final String selfId) {
        final DatabaseWrapper db = DataModel.get().getDatabase();
        ...
        Log.d("refreshSelfParticipants","conversationId:"+conversationId+" selfId:"+selfId);
        MessagingContentProvider.notifyMessagesChangedOnly(conversationId);
        MessagingContentProvider.notifyConversationMetadataChangedOnly(conversationId);
        UIIntents.get().broadcastConversationSelfIdChange(db.getContext(), conversationId, selfId);
    }
    //Bug 888919 end
    ...
    private static void refreshConversationSelfIds() {
        ...
        /* UNISOC: Modify for Bug#1403736 @{ */
        int size = conversationsToRefresh.size();
        LogUtil.d(LogUtil.BUGLE_SMART_TAG, "refreshConversationSelfIds size: " + size);

        if (size == 0 || (ActivityManager.isUserAMonkey() && size > CONVERSATION_ID_MAX)) {
            return;
        }
        /* @} */
        ...
        if (defaultSelf != null) {
            for (final String conversationId : conversationsToRefresh) {
                ...
                updateConversationSelfIdExt(conversationId, defaultSelf.getId());
                //Bug 888919 end
            }
            MessagingContentProvider.notifyWidgetMessagesAndConversationListChanged(); //Bug 888919
        }
    }

    /*smart message, begin*/
    public static void updateParticipantDataIfSmart(Context context, final ParticipantData participantData) {
        LogUtil.d(LogUtil.BUGLE_SMART_TAG, "updateParticipantDataIfSmart mHasNet: " + mHasNet);
        if (MmsConfig.supportSmartSdk() && participantData.getContactId() < 0 && mHasNet) {
            if (participantData.isAlreadySmart()) {
                LogUtil.d(LogUtil.BUGLE_SMART_TAG, "isAlreadySmart");
                return;
            }
            ISmartSdk smartSdkImpl = SmartSdkUtils.createSmartSdk();
            if (smartSdkImpl != null) {
                smartSdkImpl.getServiceInfo(context, participantData.getSendDestination(), new ServiceInfoImpl(participantData));
            }
        }
    }

    synchronized static private void refreshParticipantsById(final String partId) {
        LogUtil.d(LogUtil.BUGLE_SMART_TAG, "refreshParticipantsById, partId:" + partId);
        if (!TextUtils.isEmpty(partId) && Integer.parseInt(partId) > 0) {
            BugleDatabaseOperations.refreshConversationsForParticipant(partId);
        }
    }
    /*smart message, end*/
}
