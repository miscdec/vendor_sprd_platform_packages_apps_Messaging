...
public class MessagePartData implements Parcelable {
    ...
    private static final String[] sProjection = {
        ...
        PartColumns.ISCOMPRESSED // add for bug 725726
    };
    ...
    private static final int INDEX_COMPRESSED = 8;
    ...
    private int mWidth;
    private int mHeight;
    // add for bug 569984 begin
    private boolean mTextEqualFlag = false;
    // add for bug 569984 end
    ...
    protected void bind(final Cursor cursor) {
        ...
        mCompressed=cursor.getInt(INDEX_COMPRESSED);
        if (ContentType.isDrmType(mContentType)){
           try {
               mIsDrm = true;
               mDrmDataPath = MessagingDrmSession.get().getPath(mContentUri);
    ...
    public SQLiteStatement getInsertStatement(final DatabaseWrapper db,
        ...
        insert.bindLong(INDEX_COMPRESSED,mCompressed);
...
*/
    ...
    */
    ...
    protected MessagePartData(final Parcel in) {
        ...
        mCompressed=in.readInt();
        mIsDrm = in.readInt()==1?true:false;
        mDrmDataPath = in.readString();
        mDrmOrigContentType = in.readString();
    }
    ...
    public void writeToParcel(final Parcel dest, final int flags) {
        ...
        dest.writeInt(mCompressed);
        if(mIsDrm)
            dest.writeInt(1);
        else
            dest.writeInt(0);
        dest.writeString(mDrmDataPath);
        dest.writeString(mDrmOrigContentType);
    }
    ...
    public void debug() {
        if (isText()) {
            System.out.println("type is text, getText() = [" + getText() + "]");
        } else {
            System.out.println("type is not text, getContentType() = ["
                    + getContentType() + "],  getContentUri() = ["
                    + getContentUri() + "]");
        }
    }

    public boolean isTextEqualFlag() {
        return mTextEqualFlag;
    }

    public void setTextEqualFlag(boolean flag) {
        mTextEqualFlag = flag;
    }
    // add for bug 569984 end
    /**
