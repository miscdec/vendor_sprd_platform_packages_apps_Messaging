...
public class ConversationData extends BindableData {
    ...
    public MessageData createForwardedMessage(final ConversationMessageData message) {
        ...
        final MessageData forwardedMessage;

        /*Add by SPRD for bug 666022 start*/
        if (message.getIsMms() && message.getParts().size() > 1) {
        /*Add by SPRD for bug 666022 end*/
            forwardedMessage = new MessageData(message.getSmsMessageUri(), MessageData.PROTOCOL_MMS);//_SMIL); 923080
        } else {
            forwardedMessage = new MessageData();
        }
    ...
    public void restart(final LoaderManager loaderManager,
                     final BindingBase<ConversationData> binding) {//bug 1008873
        final Bundle args = new Bundle();
        args.putString(BINDING_ID, binding.getBindingId());
        mLoaderManager = loaderManager;
        ...
        mLoaderManager.restartLoader(SELF_PARTICIPANT_LOADER, args, mSelfParticipantLoaderCallbacks);
    }
}
