...
public class ParticipantData implements Parcelable {
    ...
    public static final long PARTICIPANT_CONTACT_ID_SERVICEPORT = -3;
    ...
    public static ParticipantData getFromRecipientEntry(final RecipientEntry recipientEntry) {
        ...
        // spread: add for bug 493591 start
        if(recipientEntry.getDestination() != null){
             pd.mSendDestination = TextUtil.replaceUnicodeDigits(recipientEntry.getDestination());
        }else{
             pd.mSendDestination = "";
             Log.i("ParticipantData", "recipientEntry.getDestination() == null");
        }
        // spread: add for bug 493591 start end
        pd.mIsEmailAddress = MmsSmsUtils.isEmailAddress(pd.mSendDestination);
        PhoneUtils nPhoneUtils = PhoneUtils.getDefault();
        pd.mNormalizedDestination = pd.mIsEmailAddress ?
                pd.mSendDestination :
                nPhoneUtils.getCanonicalBySystemLocale(pd.mSendDestination);
        pd.mDisplayDestination = pd.mIsEmailAddress ?
                pd.mNormalizedDestination :
                nPhoneUtils.formatForDisplay(pd.mSendDestination);
    ...
    public static ParticipantData getFromRawPhoneBySystemLocale(final String phoneNumber) {
        ...
        pd.mDisplayDestination = pd.mIsEmailAddress ?
                pd.mNormalizedDestination :
                PhoneUtils.getDefault().formatForDisplay(pd.mSendDestination);
        pd.maybeSetupUnknownSender();
        return pd;
    }
    ...
    public static ParticipantData getSprdFromRecipientEntry(final RecipientEntry recipientEntry,PhoneUtils phoneUtils) {
        final ParticipantData pd = new ParticipantData();
        pd.mParticipantId = null;
        pd.mSubId = OTHER_THAN_SELF_SUB_ID;
        pd.mSlotId = INVALID_SLOT_ID;
        ...
        pd.mNormalizedDestination = pd.mIsEmailAddress ?
                pd.mSendDestination :
                phoneUtils.getCanonicalBySystemLocale(pd.mSendDestination);
        /*pd.mDisplayDestination = pd.mIsEmailAddress ?
                pd.mNormalizedDestination :
                phoneUtils.formatForDisplay(pd.mSendDestination);*/
        pd.mDisplayDestination = pd.mIsEmailAddress ?
                pd.mNormalizedDestination :pd.mSendDestination;
        pd.mFullName = recipientEntry.getDisplayName();
        pd.mFirstName = null;
        pd.mProfilePhotoUri = (recipientEntry.getPhotoThumbnailUri() == null) ? null :
                recipientEntry.getPhotoThumbnailUri().toString();
    ...
    public boolean isAlreadySmart(){
        return getProfilePhotoUri()!=null && getProfilePhotoUri().startsWith("http");
    }
}
