...
public class MessageData implements Parcelable {
    private static final String[] sProjection = {
        ...
        MessageColumns.SUB_ID,
		//613227
        MessageColumns.ALARM,
    };
    ...
    private static final int INDEX_SUB_ID = 19;
	//613227
    private static final int INDEX_ALARM = 20;
    ...
    public static final String PROTOCOL_REPLACE = "isreplacesms";//for bug694631
    // Add by SPRD for bug 563344
    public static final int PROTOCOL_MMS_SMIL = 3;
    ...
    public static final int BUGLE_STATUS_OUTGOING_ALARM_TO_SEND = 10;
    // Incoming
    ...
    public static final String getStatusDescription(int status) {
        switch (status) {
            ...
            case BUGLE_STATUS_OUTGOING_ALARM_TO_SEND:
                return "OUTGOING_ALARM_TO_SEND";
    ...
    public MessageData(String smsMessageUri, int protocol) {
        this();
        mProtocol = protocol;
        mSmsMessageUri = Uri.parse(smsMessageUri);
    }
    ...
    public static boolean shouldShowLoadingDialog (final MessageData content) {
        if (content == null) {
            return false;
        }
        for (final MessagePartData part : content.getParts()) {
            ...
            if (ContentType.isVCardType(part.getContentType())
                    && uri.toSafeString().startsWith("content://com.android.contacts/contacts/as_multi_vcard")) {
                return true;
            }
        }
        return false;
     ...
     */
    public static MessageData createDraftMessage(final String conversationId, final String selfId,
            final MessageData content) {
        Log.e("smil-s", "createDraftMessage");
        ...
        if (content == null) {
            Log.e("smil-s", "createDraftMessage:content is null");
            message.mParts.add(MessagePartData.createTextMessagePart(""));
        } else {
            /* Add by SPRD for bug 563344 Start */
            if (content.mProtocol != PROTOCOL_UNKNOWN) {
                message.mProtocol = content.mProtocol;
            } else {
                message.mProtocol = PROTOCOL_UNKNOWN;
            }
            if (content.mSmsMessageUri != null) {
                message.mSmsMessageUri = content.mSmsMessageUri;
            }
            /* Add by SPRD for bug 563344 End */
            ...
            if (content.mAlarm!=0){
                message.mAlarm=content.mAlarm;
            }
            message.mMmsMessageSize = content.mMmsMessageSize;
            for (final MessagePartData part : content.getParts()) {
                if ("application/smil".equals(part.getContentType())) {
                    Log.e("smil-s", "createDraftMessage:is smil xml");
                }
                //bug887458 begin
                Uri uri = part.getContentUri();
                if (ContentType.isVCardType(part.getContentType())
                        && (uri.toSafeString().startsWith("content://com.android.contacts/contacts/as_vcard")
                        || uri.toSafeString().startsWith("content://com.android.contacts/contacts/as_multi_vcard"))) {
                    uri =  UriUtil.persistContentToScratchSpace(uri);
                    part.setContentUri(uri);
                }
                //bug887458 end
    ...
    public static MessageData createDraftMmsMessage(final String conversationId,
        ...
        Log.d("smil",
                "MessageData.createDraftMmsMessage:create:mStatus,mProtocol,mConversationId,mParticipantId,mSelfId,mMmsSubject,mReceivedTimestamp");
    ...
    /* Add by SPRD for bug 563344 Start */
    public static MessageData createDraftMmsMessage(final String conversationId,
            final String selfId, final String messageText, final String subjectText, int protocol) {
        final MessageData message = createDraftMmsMessage(conversationId, selfId, messageText, subjectText);
        if (PROTOCOL_MMS == protocol || PROTOCOL_MMS_SMIL == protocol) {
            message.mProtocol = protocol;
        }
        return message;
    }
    /* Add by SPRD for bug 563344 End */
    //for bug710962 begin
    public void setSmilProtocol(){
        mProtocol = PROTOCOL_MMS_SMIL;
    }
    //for bug710962 end
    /**
    ...
    /* Add by SPRD for Bug:523616 2016.01.19 Start */
    public ArrayList<MessagePartData> getPart() {
        return mParts;
    }
    /* Add by SPRD for Bug:523616 2016.01.19 End */
    ...
    public void bind(final Cursor cursor) {
        ...
        mAlarm = cursor.getLong(INDEX_ALARM);
    }
    ...
    public SQLiteStatement getInsertStatement(final DatabaseWrapper db) {
        LogUtil.i(LogUtil.BUGLE_ALARM_TAG,"getInsertStatement:mAlarm:"+mAlarm);
        ...
        insert.bindLong(INDEX_SUB_ID, mSubId);
        insert.bindLong(INDEX_ALARM, mAlarm);
    ...
    //613227
    public final void setAlarm(final long alarm) {
        mAlarm = alarm;
    }
    ...
    /*Add by SPRD for bug585755  2016.08.05 Start*/
    public boolean getMmsIsExpired(){
        if (mMmsExpiry < System.currentTimeMillis()) {
            return true;
        }else{
            return false;
        }
    }
    /*Add by SPRD for bug585755  2016.08.05 End*/
    ...
    public final boolean getIsMms() {
        return mProtocol == MessageData.PROTOCOL_MMS
                ...
                || mProtocol == MessageData.PROTOCOL_MMS_SMIL;  // Add for bug 563344
    }
    ...
    public final void updateSendingMessage(final String conversationId, final Uri messageUri,
        ...
        /* Add by SPRD for bug 563344 Start */
        if (PROTOCOL_MMS_SMIL == mProtocol) {
            mProtocol = PROTOCOL_MMS;
        }
        /* Add by SPRD for bug 563344 End */
    }
    ...
    public final void markMessageAlarm() {
        // 613227
        mStatus = BUGLE_STATUS_OUTGOING_ALARM_TO_SEND;
    ...
    public void setMmsMessageSize(long size) {
        mMmsMessageSize = size;
    }

    public long getMmsMessageSize() {
        return mMmsMessageSize;
    }

    public boolean hasContent() {
        // add new function for empty msg begin
         if((MmsConfig.get(mSubId).getFinalSendEmptyMessageFlag() == 0)
                 || (MmsConfig.get(mSubId).getFinalSendEmptyMessageFlag() == 1)
                 || (MmsConfig.get(mSubId).getFinalSendEmptyMessageFlag() == 2)){
             if(getIsMms()){
                ...
                if ((TextUtils.getTrimmedLength(getMessageText()) > 0)||(mAlarm!=0)) {
                    return true;
                } else {
                    return false;
                }
                ...
                boolean result=!TextUtils.isEmpty(mMmsSubject) ||
                    getFirstAttachment() != null ||
                    !TextUtils.isEmpty(getMessageText())||
                    (mAlarm!=0);
                LogUtil.d(LogUtil.BUGLE_TAG,"hasContent result:["+result);
                return result;
         }
    ...
    //bug 633234 : six queues begin
    public final void bindSubId(final int subId) {
        mSubId = subId;
    }
    //bug 633234 : six queues end
