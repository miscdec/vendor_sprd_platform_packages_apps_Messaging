...
public class ConversationMessageData {
    ...
    public void bind(final Cursor cursor) {
        ...
        if (mParts != null) {
            for (MessagePartData part : mParts) {
                if (!mIsDrmLocked) {
                    mIsDrmLocked = part.isDrmType();
                } else {
                    break;
                }
            }
    ...
    public String getPartsUri(final Cursor cursor) {
        mPartsUri = cursor.getString(INDEX_PARTS_CONTENT_URIS);
        return mPartsUri;
    }
    ...
    static MessagePartData makePartData(
        ...
        if (ContentType.isTextType(contentType)) {
            ...
            }else{ // add for smil begin
                MessagePartData attachmentPart = MessagePartData.createEmptyMessagePart();
                return attachmentPart;
            }// add for smil end
        }
    }
    ...
    public final String getPartsUri() { return mPartsUri; }
    ...
    public static final String getSingleMessageQuerySql() {
        return SINGLE_MESSAGES_QUERY_SQL // Inject the message id
    ...
    // add for bug 566254 begin
    public boolean getCanPreviewSmilData() {
        return (mStatus == MessageData.BUGLE_STATUS_OUTGOING_COMPLETE
                || mStatus == MessageData.BUGLE_STATUS_OUTGOING_DELIVERED
                || mStatus == MessageData.BUGLE_STATUS_OUTGOING_FAILED
                || mStatus == MessageData.BUGLE_STATUS_OUTGOING_FAILED_EMERGENCY_NUMBER
    ...
    public boolean getCanForwardMessage() {
        ...
        return (mStatus == MessageData.BUGLE_STATUS_OUTGOING_COMPLETE ||
                ...
                /* Add for bug  583050 {@ */
                mStatus == MessageData.BUGLE_STATUS_OUTGOING_DELIVERED /* @} */||
                mStatus == MessageData.BUGLE_STATUS_OUTGOING_ALARM_TO_SEND);
    }
    ...
    private static final String CONVERSATION_MESSAGES_QUERY_PROJECTION_SQL =
            ...
            + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.SUB_ID
            + " as " + ConversationMessageViewColumns.SUB_ID + ", "
            + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.ALARM
            + " as " + ConversationMessageViewColumns.ALARM + ", "
    ...
    // bug 478514: Add for MmsFolderView Feature -- Begin
    private static final String SINGLE_MESSAGES_QUERY_FROM_WHERE_SQL = " FROM "
            + DatabaseHelper.MESSAGES_TABLE + " LEFT JOIN " + DatabaseHelper.PARTS_TABLE + " ON ("
            + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns._ID + "="
            + DatabaseHelper.PARTS_TABLE + "." + PartColumns.MESSAGE_ID + ") " + " LEFT JOIN "
            + DatabaseHelper.PARTICIPANTS_TABLE + " ON (" + DatabaseHelper.MESSAGES_TABLE + '.'
            + MessageColumns.SENDER_PARTICIPANT_ID + '=' + DatabaseHelper.PARTICIPANTS_TABLE + '.'
            + ParticipantColumns._ID + ")"
            + " WHERE (";
    private static final String SINGLE_MESSAGES_QUERY_SQL = "SELECT "
            + CONVERSATION_MESSAGES_QUERY_PROJECTION_SQL
            + SINGLE_MESSAGES_QUERY_FROM_WHERE_SQL;
    // bug 478514: Add for MmsFolderView Feature -- End
    ...
    interface ConversationMessageViewColumns extends BaseColumns {
        ...
        static final String ALARM =MessageColumns.ALARM;
        static final String SUB_ID = MessageColumns.SUB_ID;
    }
    ...
    private static final int INDEX_ALARM                        = sIndexIncrementer++;
    ...
    private static String[] sProjection = {
        ...
        ConversationMessageViewColumns.SUB_ID,
        ConversationMessageViewColumns.ALARM,
    ...
    private boolean isCanSildeshowStatus(){
         return (mStatus==MessageData.BUGLE_STATUS_INCOMING_COMPLETE
                      || mStatus==MessageData.BUGLE_STATUS_OUTGOING_COMPLETE);
    }

    //fix bug 598897  start
    private boolean isPlayMedia(final MessagePartData part) {
        if (part.isVideo() || part.isAudio() || part.isImage()
                || (part.getContentType()!=null&&part.getContentType().equalsIgnoreCase("text/plain"))) {
            return true;
        }
        return false;
    }
    ...
    public boolean isCanSildeshow() {
        if (isCanSildeshowStatus()) {
            if (mParts != null && !mParts.isEmpty()) {
                for (final MessagePartData part : mParts) {
                    // fix bug 598897 start
                    if (part.isAttachment() && isPlayMedia(part)) {
                        return true;
                    }
                    // fix bug 598897 end
                }
    ...
    public static ConversationMessageData queryConversationMessagesById(final String conversationId,
                                                                        final String messageId) {
        final String[] queryArgs = {conversationId, messageId};
        Cursor c = null;
        ConversationMessageData cmd = new ConversationMessageData();
        try {
            String rawSql = CONVERSATION_MESSAGES_QUERY_SQL
                    + " AND "
                    + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns.CONVERSATION_ID + "=?"
                    + " AND "
                    + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns._ID + "=?)";
            c = Factory.get().getDataModel().getDatabase().rawQuery(rawSql, queryArgs);
            if (c != null && c.moveToFirst()) {
                cmd.bind(c);
            }
        } catch (Exception e) {
            SprdLogUtil.dump("conversationId", conversationId, "Exception", e.toString());
        } finally {
            if (c != null) {
                c.close();
            }
    ...
    /*smart message, begin*/
    public boolean isSmartMessage(){
        return getIsIncoming() && (getSenderProfilePhotoUri() != null && getSenderProfilePhotoUri().toString().startsWith("http"));
    }
    /*smart message, end*/
}
