...
public class MessagingContentProvider extends ContentProvider {
    ...
    private static final int MESSAGING_URI_SEARCH = 150;
    ...
    static {
        ...
        // bug 495194 : add for search feature end
        sURIMatcher.addURI(AUTHORITY, "search", MESSAGING_URI_SEARCH);
    }
    ...
    //Bug 888919 begin
    public static void notifyWidgetMessagesAndConversationListChanged() {
        final Context context = Factory.get().getApplicationContext();
        notifyConversationListChangedOnly();
        if(getWidgetStatusSharedPref().getBoolean(WidgetConversationProvider.CONVERSATION_WIDGET_CREATE, false)){
            WidgetConversationProvider.notifyMessagesChanged(context, null);
        }
        if(getWidgetStatusSharedPref().getBoolean(BugleWidgetProvider.BUGLE_WIDGET_CREATE, false)){
            BugleWidgetProvider.notifyConversationListChanged(context);
        }
    }
    //Bug 888919 end
    ...
    //Bug 888919 begin
    public static void notifyMessagesChangedOnly(final String conversationId) {
        final Uri uri = buildConversationMessagesUri(conversationId);
        final Context context = Factory.get().getApplicationContext();
        final ContentResolver cr = context.getContentResolver();
        cr.notifyChange(uri, null);
    ...
    //Bug 888919 begin
    public static void notifyConversationMetadataChangedOnly(final String conversationId) {
        final Uri uri = buildConversationMetadataUri(conversationId);
        final ContentResolver cr = Factory.get().getApplicationContext().getContentResolver();
        cr.notifyChange(uri, null);
    }
    ...
    //Bug 888919 begin
    public static void notifyConversationListChangedOnly() {
        final Context context = Factory.get().getApplicationContext();
        final ContentResolver cr = context.getContentResolver();
        cr.notifyChange(CONVERSATIONS_URI, null);
        notifyMessageListViewChanged();
    ...
    /* Add by SPRD for optimization broadcast 2016.09.18 Start */
    private static SharedPreferences getWidgetStatusSharedPref(){
        return Factory.get().getApplicationContext().getSharedPreferences(BugleWidgetProvider.WIDGET_STATE, Context.MODE_PRIVATE);
    }
    /* Add by SPRD for optimization broadcast 2016.09.18 End */
    ...
    public Cursor query(final Uri uri, final String[] projection, String selection,
        ...
        switch (match) {
            ...
            //Bug 880287
            case MESSAGING_URI_SEARCH:
                String searchString = uri.getQueryParameter("pattern");
                return com.android.messaging.datamodel.BugleDatabaseOperations.QureyMessaging(searchString);
            //Bug 880287
    ...
    public boolean onCreate() {
        ...
        if (mHandler == null) {
            mSimThread = new HandlerThread("SimThread");
            mSimThread.start();
            mSimLooper = mSimThread.getLooper();
            mHandler = new SimHandler(mSimLooper);
        }
        return true;
    }
    /* Bug 489223 begin*/
  ...
  //bug 554851 :Begin
    ...
    private class  SimHandler extends Handler {
        public SimHandler(Looper looper) {
            super(looper);
        }
