...
public class MediaScratchFileProvider extends FileProvider {
    ...
    public static Uri buildMediaScratchSpaceUri(final String extension) {
	    ...
	    //LogUtil.e(TAG, "buildMediaScratchSpaceUri path is  " + file.getAbsolutePath());
        return uri;
    }

    public static Uri buildMediaScratchSpaceUriForDrm(final String fileName, final String extension) {
        final Uri uri = FileProvider.buildFileUriForDrm(AUTHORITY, extension, fileName);
        final File file = getFileWithExtension(uri.getPath(), extension);
        if (!ensureFileExists(file)) {
            LogUtil.e(TAG, "Failed to create temp file " + file.getAbsolutePath());
        }
        LogUtil.d(TAG, "buildMediaScratchSpaceUriForDrm path is  " + file.getAbsolutePath());
        return uri;
    }
    ...
    private static File getFileWithExtension(final String path, final String extension) {
        ...
        if(null != Factory.get() ){
            context = Factory.get().getApplicationContext();
        }else {
            return null;
        }
        //bug 1205933 end
