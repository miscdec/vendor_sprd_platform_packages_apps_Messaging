...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String SIM_MESSAGES_TABLE = "sim_messages";

    // Views
    static final String DRAFT_PARTS_VIEW = "draft_parts_view";
    static final String SIM_MESSAGE_LIST_VIEW = "sim_message_list_view";
    ...
    private static final String CONVERSATIONS_ID_INDEX_SQL =
            "CREATE INDEX index_" + CONVERSATIONS_TABLE + "_" + ConversationColumns._ID
            + " ON " +  CONVERSATIONS_TABLE
    ...
    public static class MessageColumns implements BaseColumns {
        ...
        /* fanzhiri add for two list to send */
    ...
    private static final String CREATE_MESSAGES_TABLE_SQL =
            "CREATE TABLE " + MESSAGES_TABLE + " ("
                    ...
                    + MessageColumns.SUB_ID + " INT, "
                    + MessageColumns.ALARM + " INT DEFAULT(0), "
    ...
    public static class PartColumns implements BaseColumns {
        ...
        public static final String ISCOMPRESSED = "iscompressed";//add for bug 725726
    ...
    private static final String CREATE_PARTS_TABLE_SQL =
            "CREATE TABLE " + PARTS_TABLE + "("
                    ...
                    + MessagingContentProvider.UNSPECIFIED_SIZE + "),"
                    + PartColumns.ATTACHMENT_SIZE + " DOUBLE DEFAULT("
                    + MessagingContentProvider.UNSPECIFIED_SIZE + "),"
                    + PartColumns.TIMESTAMP + " INT, "
                    + PartColumns.CONVERSATION_ID + " INT NOT NULL,"
                    + PartColumns.ISCOMPRESSED + " INT DEFAULT("
                    + MessagingContentProvider.UNSPECIFIED_SIZE + "),"
    ...
    static final String DRAFT_PARTS_VIEW_SQL = "CREATE VIEW " +
            ...
            + " as " + PartColumns.HEIGHT + ", "
                        + PARTS_TABLE + '.' + PartColumns.ATTACHMENT_SIZE
            + " as " + PartColumns.ATTACHMENT_SIZE + ", "
                        + PARTS_TABLE + '.' + PartColumns.ISCOMPRESSED
            + " as " + PartColumns.ISCOMPRESSED + ", "
    ...
    static final String MESSAGE_LIST_VIEW = "message_list_view";
    static final String MESSAGES_ALL_COLUMNS = MESSAGES_TABLE + ".*";
    static final String PARTS_ALL_COLUMNS = PARTS_TABLE + ".*";
    static final String TEXT_PLAIN_TYPE = "'text/plain'";
    static final String VIDEOS_TYPE = "'video%'";
    static final String IMAGES_TYPE = "'image%'";
    static final String AUDIOS_TYPE = "'audio%'";
    public static class MessageListViewColumns implements BaseColumns {
        public static final String _ID = MessageColumns._ID;
        static final String CONVERSATION_ID = MessageColumns.CONVERSATION_ID;
        static final String SENDER_PARTICIPANT_ID = MessageColumns.SENDER_PARTICIPANT_ID;
        ...
        static final String MMS_TRANSACTION_ID = MessageColumns.MMS_TRANSACTION_ID;
        static final String MMS_CONTENT_LOCATION = MessageColumns.MMS_CONTENT_LOCATION;
        ...
        static final String RETRY_START_TIMESTAMP = MessageColumns.RETRY_START_TIMESTAMP;
        ...
        static final String DRAFT_PREVIEW_CONTENT_TYPE = ConversationColumns.DRAFT_PREVIEW_CONTENT_TYPE;
        //add for bug688310 end
        static final String OTHER_PARTICIPANT_NORMALIZED_DESTINATION = ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION;
        static final String PARTICIPANT_COUNT = ConversationColumns.PARTICIPANT_COUNT;
        static final String SUB_ID = ParticipantColumns.SUB_ID;
        static final String SIM_SLOT_ID = ParticipantColumns.SIM_SLOT_ID;
        static final String SUBSCRIPTION_NAME = ParticipantColumns.SUBSCRIPTION_NAME;
        static final String SUBSCRIPTION_COLOR = ParticipantColumns.SUBSCRIPTION_COLOR;
        static final String TEXT = PartColumns.TEXT;
        static final String CONTENT_URI = PartColumns.CONTENT_URI;
        static final String CONTENT_TYPE = PartColumns.CONTENT_TYPE;
        static final String DISPLAY_DESTINATION = ParticipantColumns.DISPLAY_DESTINATION;
        static final String FULL_NAME = ParticipantColumns.FULL_NAME;
        static final String FIRST_NAME = ParticipantColumns.FIRST_NAME;
    }
    static final String MESSAGES_JOIN_CONVERSATIONS_JOIN_PARTICIPANTS_QUERY_SQL =
            "SELECT T.*" + ", " + PARTICIPANTS_TABLE + '.'
            + ParticipantColumns.SUB_ID + " AS " + ParticipantColumns.SUB_ID + ", "
            + PARTICIPANTS_TABLE + '.' + ParticipantColumns.SIM_SLOT_ID + " AS "
            + ParticipantColumns.SIM_SLOT_ID + ", " + PARTICIPANTS_TABLE + '.'
            + ParticipantColumns.SUBSCRIPTION_NAME + " AS " + ParticipantColumns.SUBSCRIPTION_NAME
            + ", " + PARTICIPANTS_TABLE + '.' + ParticipantColumns.SUBSCRIPTION_COLOR + " AS "
            + ParticipantColumns.SUBSCRIPTION_COLOR + " FROM ((SELECT " + MESSAGES_ALL_COLUMNS + ", "
            + CONVERSATIONS_TABLE + '.' + ConversationColumns.NAME + " AS "
            + ConversationColumns.NAME + ", "
            + CONVERSATIONS_TABLE + '.' + ConversationColumns.DRAFT_SNIPPET_TEXT + " AS "
            + ConversationColumns.DRAFT_SNIPPET_TEXT + ", "
            + CONVERSATIONS_TABLE + '.' + ConversationColumns.DRAFT_PREVIEW_URI + " AS "
            + ConversationColumns.DRAFT_PREVIEW_URI + ", "
            + CONVERSATIONS_TABLE + '.' + ConversationColumns.DRAFT_PREVIEW_CONTENT_TYPE + " AS "
            + ConversationColumns.DRAFT_PREVIEW_CONTENT_TYPE + ", "
            + CONVERSATIONS_TABLE + '.'
            + ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION + " AS "
            + ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION + ", "
            + CONVERSATIONS_TABLE + '.' + ConversationColumns.PARTICIPANT_COUNT + " AS "
            + ConversationColumns.PARTICIPANT_COUNT + " FROM " + MESSAGES_TABLE + " INNER JOIN "
            + CONVERSATIONS_TABLE + " ON " + MESSAGES_TABLE + '.' + MessageColumns.CONVERSATION_ID
            + '=' + CONVERSATIONS_TABLE + '.' + ConversationColumns._ID + ") AS T) INNER JOIN "
            + PARTICIPANTS_TABLE + " ON T" + '.' + MessageColumns.SELF_PARTICIPANT_ID + '='
            + PARTICIPANTS_TABLE + '.' + ParticipantColumns._ID;
    static final String PARTS_TEXT_ALL_QUERY_SQL = "SELECT " + PartColumns._ID + ", "
            + PartColumns.MESSAGE_ID + ", " + PartColumns.TEXT + " FROM (" + " SELECT "
            + PARTS_ALL_COLUMNS + " FROM " + PARTS_TABLE + " WHERE " + PARTS_TABLE + '.'
            + PartColumns.CONTENT_TYPE + '=' + TEXT_PLAIN_TYPE + " ORDER BY "
            + PartColumns.MESSAGE_ID + ", " + PartColumns._ID + " DESC)" + " GROUP BY "
            + PartColumns.MESSAGE_ID;
    static final String PARTS_MULTIMEDIA_ALL_QUERY_SQL = "SELECT " + PartColumns._ID + ", "
            + PartColumns.MESSAGE_ID + "," + PartColumns.CONTENT_URI + ", "
            + PartColumns.CONTENT_TYPE + " FROM ( SELECT * FROM " + PARTS_TABLE + " WHERE "
            + PARTS_TABLE + '.' + PartColumns.CONTENT_TYPE + " LIKE " + IMAGES_TYPE + " OR "
            + PARTS_TABLE + '.' + PartColumns.CONTENT_TYPE + " LIKE " + VIDEOS_TYPE + " OR "
            + PARTS_TABLE + '.' + PartColumns.CONTENT_TYPE + " LIKE " + AUDIOS_TYPE + " ORDER BY "
            + PartColumns.MESSAGE_ID + ", " + PartColumns._ID + " DESC) " + " GROUP BY "
            + PartColumns.MESSAGE_ID;
    static final String PARTS_BOTH_TEXT_AND_MULTIMEDIA_QUERY_SQL = "SELECT T1" + '.'
            + PartColumns.MESSAGE_ID + " AS " + PartColumns.MESSAGE_ID + ", T1" + '.'
            + PartColumns.TEXT + " AS " + PartColumns.TEXT + ", T2" + '.' + PartColumns.CONTENT_URI
            + " AS " + PartColumns.CONTENT_URI + ", T2" + '.' + PartColumns.CONTENT_TYPE + " AS "
            + PartColumns.CONTENT_TYPE + " FROM (" + PARTS_TEXT_ALL_QUERY_SQL + ") AS T1, ("
            + PARTS_MULTIMEDIA_ALL_QUERY_SQL + ") AS T2 WHERE " + "T1" + '.'
            + PartColumns.MESSAGE_ID + '=' + "T2" + '.' + PartColumns.MESSAGE_ID;
    static final String PARTS_TEXT_NOTIN_MULTIMEDIA_QUERY_SQL = "SELECT " + PartColumns.MESSAGE_ID
            + ", " + PartColumns.TEXT + ", " + PartColumns.CONTENT_URI + ", "
            + PartColumns.CONTENT_TYPE + " FROM ( SELECT * FROM " + PARTS_TABLE + " WHERE "
            + PartColumns.MESSAGE_ID + " NOT IN ( SELECT " + PARTS_TABLE + '.'
            + PartColumns.MESSAGE_ID + " FROM " + PARTS_TABLE + " WHERE " + PARTS_TABLE + '.'
            + PartColumns.CONTENT_TYPE + " LIKE " + IMAGES_TYPE + " OR " + PARTS_TABLE + '.'
            + PartColumns.CONTENT_TYPE + " LIKE " + VIDEOS_TYPE + " OR " + PARTS_TABLE + '.'
            + PartColumns.CONTENT_TYPE + " LIKE " + AUDIOS_TYPE + " GROUP BY "
            + PartColumns.MESSAGE_ID + ") ORDER BY " + PartColumns.MESSAGE_ID + ", "
            + PartColumns._ID + " DESC) GROUP BY " + PartColumns.MESSAGE_ID;
    static final String PARTS_MULTIMEDIA_NOTIN_TEXT_QUERY_SQL = "SELECT " + PartColumns.MESSAGE_ID
            + ", " + PartColumns.TEXT + ", " + PartColumns.CONTENT_URI + ", "
            + PartColumns.CONTENT_TYPE + " FROM (SELECT * FROM " + PARTS_TABLE + " WHERE "
            + PartColumns.MESSAGE_ID + " NOT IN (SELECT " + PARTS_TABLE + '.'
            + PartColumns.MESSAGE_ID + " FROM " + PARTS_TABLE + " WHERE " + PARTS_TABLE + '.'
            + PartColumns.CONTENT_TYPE + '=' + TEXT_PLAIN_TYPE + " GROUP BY "
            + PartColumns.MESSAGE_ID + ") ORDER BY " + PartColumns.MESSAGE_ID + ", "
            + PartColumns._ID + " DESC) GROUP BY " + PartColumns.MESSAGE_ID;
    static final String PARTICIPANTS_JOIN_CONVERSATIONS_PARTICIPANTS_QUERY_SQL = "SELECT "
            + CONVERSATION_PARTICIPANTS_TABLE + '.'
            + ConversationParticipantsColumns.CONVERSATION_ID + ", " + PARTICIPANTS_TABLE + '.'
            + ParticipantColumns.DISPLAY_DESTINATION + ", " + PARTICIPANTS_TABLE + '.'
            + ParticipantColumns.FULL_NAME + ", " + PARTICIPANTS_TABLE + '.'
            + ParticipantColumns.FIRST_NAME + " FROM " + PARTICIPANTS_TABLE + " INNER JOIN "
            + CONVERSATION_PARTICIPANTS_TABLE + " ON " + CONVERSATION_PARTICIPANTS_TABLE + '.'
            + ConversationParticipantsColumns.PARTICIPANT_ID + '=' + PARTICIPANTS_TABLE + '.'
            + ParticipantColumns._ID + " GROUP BY " + CONVERSATION_PARTICIPANTS_TABLE + '.'
            + ConversationParticipantsColumns.CONVERSATION_ID;
    static final String MESSAGE_LIST_VIEW_SQL = "CREATE VIEW "+ MESSAGE_LIST_VIEW + " AS SELECT T5.*, T4."
            + ParticipantColumns.DISPLAY_DESTINATION + " AS "
            + ParticipantColumns.DISPLAY_DESTINATION + ", T4." + ParticipantColumns.FULL_NAME
            + " AS " + ParticipantColumns.FULL_NAME + ", T4." + ParticipantColumns.FIRST_NAME
            + " AS " + ParticipantColumns.FIRST_NAME + " FROM (SELECT " + " T0.*, T3."
            + MessageListViewColumns.TEXT + " AS " + MessageListViewColumns.TEXT + ", T3."
            + MessageListViewColumns.CONTENT_URI + " AS " + MessageListViewColumns.CONTENT_URI
            + " , T3." + MessageListViewColumns.CONTENT_TYPE + " AS "
            + MessageListViewColumns.CONTENT_TYPE + " FROM ("
            + MESSAGES_JOIN_CONVERSATIONS_JOIN_PARTICIPANTS_QUERY_SQL + ") AS T0 LEFT JOIN ("
            + PARTS_BOTH_TEXT_AND_MULTIMEDIA_QUERY_SQL + " UNION "
            + PARTS_TEXT_NOTIN_MULTIMEDIA_QUERY_SQL + " UNION "
            + PARTS_MULTIMEDIA_NOTIN_TEXT_QUERY_SQL + ") AS T3 ON T0." + MessageColumns._ID + '='
            + "T3." + PartColumns.MESSAGE_ID + " ORDER BY "
            + MessageListViewColumns.RECEIVED_TIMESTAMP + " DESC) AS T5 INNER JOIN ("
            + PARTICIPANTS_JOIN_CONVERSATIONS_PARTICIPANTS_QUERY_SQL
            + ") AS T4 ON T5.conversation_id=T4.conversation_id";
    // bug 478514: Add for MmsFolderView Feature -- End

    // SIM Messages table SQL(the same structure as table Messages)
    static final String SIM_MESSAGES_ALL_COLUMNS = SIM_MESSAGES_TABLE + ".*";
    private static final String CREATE_SIM_MESSAGES_TABLE_SQL =
            "CREATE TABLE " + SIM_MESSAGES_TABLE + " ("
                    ...
                    + MessageColumns.PROTOCOL + " INT DEFAULT(0), "
                    + MessageColumns.STATUS + " INT DEFAULT(0), "
                    + MessageColumns.SEEN + " INT DEFAULT(1), "
                    + MessageColumns.READ + " INT DEFAULT(0), "
                    + MessageColumns.SMS_MESSAGE_URI + " TEXT, "
                    + MessageColumns.SMS_PRIORITY + " INT DEFAULT(0), "
                    + MessageColumns.SMS_MESSAGE_SIZE + " INT DEFAULT(0), "
    ...
    static final String SIM_MESSAGES_JOIN_CONVERSATIONS_JOIN_PARTICIPANTS_QUERY_SQL =
            "SELECT T.*" + ", " + PARTICIPANTS_TABLE + '.' + ParticipantColumns.SIM_SLOT_ID + " AS "
                    + ParticipantColumns.SIM_SLOT_ID + ", " + PARTICIPANTS_TABLE + '.'
                    + ParticipantColumns.SUBSCRIPTION_NAME + " AS " + ParticipantColumns.SUBSCRIPTION_NAME
                    + ", " + PARTICIPANTS_TABLE + '.' + ParticipantColumns.SUBSCRIPTION_COLOR + " AS "
                    + ParticipantColumns.SUBSCRIPTION_COLOR + " FROM ((SELECT " + SIM_MESSAGES_ALL_COLUMNS + ", "
                    + CONVERSATIONS_TABLE + '.'
                    + ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION + " AS "
                    + ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION + ", "
                    + CONVERSATIONS_TABLE + '.' + ConversationColumns.PARTICIPANT_COUNT + " AS "
                    + ConversationColumns.PARTICIPANT_COUNT + " FROM " + SIM_MESSAGES_TABLE + " LEFT JOIN "
                    + CONVERSATIONS_TABLE + " ON " + SIM_MESSAGES_TABLE + '.' + MessageColumns.CONVERSATION_ID
                    + '=' + CONVERSATIONS_TABLE + '.' + ConversationColumns._ID + ") AS T) LEFT JOIN "
                    + PARTICIPANTS_TABLE + " ON T" + '.' + ParticipantColumns.SUB_ID + '='
                    + PARTICIPANTS_TABLE + '.' + ParticipantColumns.SUB_ID;

    static final String SIM_MESSAGE_LIST_VIEW_SQL = "CREATE VIEW "+ SIM_MESSAGE_LIST_VIEW + " AS SELECT T5.*, T4."
            ...
            + " , T3." + MessageListViewColumns.CONTENT_TYPE + " AS "
            + MessageListViewColumns.CONTENT_TYPE + " FROM ("
            + SIM_MESSAGES_JOIN_CONVERSATIONS_JOIN_PARTICIPANTS_QUERY_SQL + ") AS T0 LEFT JOIN ("
            + PARTS_TEXT_NOTIN_MULTIMEDIA_QUERY_SQL + ") AS T3 ON T0." + MessageColumns._ID + '='
            + "T3." + PartColumns.MESSAGE_ID + " ORDER BY "
            + MessageListViewColumns.RECEIVED_TIMESTAMP + " DESC) AS T5 LEFT JOIN ("
            + PARTICIPANTS_JOIN_CONVERSATIONS_PARTICIPANTS_QUERY_SQL
            + ") AS T4 ON T5.conversation_id=T4.conversation_id";
    ...
    private static final String[] CREATE_VIEW_SQLS = new String[] {
        ...
        // bug 478514: Add for MmsFolderView Feature -- Begin
        MESSAGE_LIST_VIEW_SQL,
        // bug 478514: Add for MmsFolderView Feature -- End
        SIM_MESSAGE_LIST_VIEW_SQL,
    };
    ...
    public static void rebuildTables(final SQLiteDatabase db) {
        Log.d("DatabaseHelper", "rebuildTables");
    ...
    public static void upgradeDatabaseToVersion2(final SQLiteDatabase db) {
        Log.d("DatabaseHelper", "upgradeDatabaseToVersion2");
        db.execSQL("ALTER TABLE messages add COLUMN alarm INTEGER DEFAULT 0");
    }
    //add for bug688310 begin
    public static void upgradeDatabaseToVersion3(final SQLiteDatabase db) {
        Log.d("DatabaseHelper", "upgradeDatabaseToVersion3");
        dropView(db,MESSAGE_LIST_VIEW,true);
        rebuildView(db,MESSAGE_LIST_VIEW,MESSAGE_LIST_VIEW_SQL);
    }
    //add for bug688310 end
    ...
    public static void upgradeDatabaseToVersion4(final SQLiteDatabase db) {
        ...
        rebuildView(db,DRAFT_PARTS_VIEW,DRAFT_PARTS_VIEW_SQL);
        db.execSQL("ALTER TABLE parts add COLUMN iscompressed INTEGER DEFAULT -1");
    }
    ...
    private static void createDatabase(final SQLiteDatabase db) {
        Log.d("DatabaseHelper", "createDatabase");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.d("DatabaseHelper", "onUpgrade, db.version:" + db.getVersion() + ", oldVersion:" + oldVersion + ", newVersion:" + newVersion);
