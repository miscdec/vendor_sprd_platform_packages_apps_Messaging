...
public class MessageTextStats {
    ...
    public void updateMessageTextStats(final int selfSubId, final String messageText) {
        ...
        String msgText = messageText + getSignatrueText();
        /*final */int[] params;
        int encodeType = -1;
        String encodeTypeString = MmsConfig.getEncodeType();
        LogUtil.d(LogUtil.BUGLE_TAG+"-encodeTypeLog", "MessageTextStats:000 encodeTypeString="+ encodeTypeString);
         if ("-1".equals(encodeTypeString)){
            encodeType = MmsConfig.get(ParticipantData.DEFAULT_SELF_SUB_ID).getEncodePrefeStatus();/*modified for Bug 724272*/
            if (-1!= encodeType) {
                SystemAdapter.getInstance().setProperty(MmsConfig.SMS_ENCODE_TYPE, String.valueOf(encodeType));
            }
            LogUtil.d(LogUtil.BUGLE_TAG+"-encodeTypeLog", "updateMessageTextStats encodeType = " + encodeType );
        } else{
            encodeType = Integer.parseInt(encodeTypeString);
            SystemAdapter.getInstance().setProperty(MmsConfig.SMS_ENCODE_TYPE, String.valueOf(encodeType));/*Bug 694629*/
        }
        LogUtil.d(LogUtil.BUGLE_TAG+"-encodeTypeLog", "MessageTextStats  encodeType:" + encodeType);

        /*Add by SPRD for Bug:597117 Encode Type feature Start*/
        params = SmsMessage.calculateLength(msgText, encodeType == 1);
