    ...
    protected Object executeAction() {
        ...
        if (sentByPlatform) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                if (responseImportant) {
                    ...
                    if (sendConf != null) {
                        ...
                        if(status == MmsUtils.MMS_REQUEST_SUCCEEDED) {
                            if(MmsUtils.isEnabelMmsSendReport(subId)) {
                                //UiUtils.showToastAtBottom( context.getString(R.string.host_app_name) + context.getString(R.string.message_outgoing_successful));
                                ThreadUtil.getMainThreadHandler().post(new Runnable() {
                                    @Override
                                    public void run() {
                                        Toast.makeText(context,
                                                context.getString(R.string.host_app_name) + context.getString(R.string.message_outgoing_successful), Toast.LENGTH_LONG).show();
                                    }
                                });
                            }
                        }
                ...
                // sprd: 596495 fdn feature
                if (resultCode == SmsManagerEx.MMS_ERROR_FDN_CHECKED_CONTACRS) {
