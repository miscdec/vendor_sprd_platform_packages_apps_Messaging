    ...
    protected Object executeAction() {
        ...
        try {
            ...
            if (hasSpecificConversation) {
                 // Mark all messages in specific thread as seen in telephony db by 856985 begin
                final long threadId = BugleDatabaseOperations.getThreadId(db, conversationId);
                if (threadId != -1) {
                    MmsUtils.updateSmsSeenStatus(threadId, Long.MAX_VALUE);
                }//by 856986 end
                // Mark all messages in specific thread as seen in local db
                ...
                // Mark all messages in all thread as seen in telephony db by 856985 begin
                final ContentResolver resolver = Factory.get().getApplicationContext().getContentResolver();
                resolver.update(Sms.CONTENT_URI,  values, Sms.SEEN + " !=1", null/*selectionArgs*/);
                resolver.update(Mms.CONTENT_URI,  values, Mms.SEEN + " !=1", null/*selectionArgs*/);
                //by 856985 end
                //Mark all messages in all thread as seen in local db
