...
public class ReadDraftDataAction extends Action implements Parcelable {
    ...
    protected Object executeAction() {
        ...
        if (conversation != null) {
            ...
            //bug#574981 start
            if(message != null && message.getPart().size() > 0) {
                // add for bug 725726 start
                boolean shouldCompress= false;
                for(final MessagePartData part : message.getPart()){
                        if(part.shouldCompress()){
                            Log.d("andy","picture should compress");
                            shouldCompress=true;
                            part.setIsCompressed(1);
                        }
                }
                ...
                long attachmentSize = MmsUtils.getAttachmentsLength(context, message.getPart());
                message.setMmsMessageSize(attachmentSize);
            }
            //bug#574981 end.
