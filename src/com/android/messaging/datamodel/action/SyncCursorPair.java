    ...
    long scan(final int maxMessagesToScan,
        ...
        while (localCount + remoteCount < maxMessagesToScan && smsToAdd.size()
            ...
            if (remoteMessage == null && localMessage == null) {
                ...
                if(remoteMessage == null && localMessage != null) {
                    LogUtil.d(LogUtil.BUGLE_ALARM_TAG, "SyncCursorPair scan messagesToDelete.add remoteMessage == null");
                }else{
                    LogUtil.d(LogUtil.BUGLE_ALARM_TAG, "SyncCursorPair scan messagesToDelete.add lm:["+localMessage.getTimestampInMillis()+"] rm:["+remoteMessage.getTimestampInMillis());
                
                }
                LogUtil.d(LogUtil.BUGLE_ALARM_TAG, "SyncCursorPair scan messagesToDelete.add local message that is not in remote db");
                ...
                if ((remoteMessagePeek == null ||
                    ...
                    if (!remoteMessage.equals(localMessage)) {
                        ...
                        LogUtil.d(LogUtil.BUGLE_ALARM_TAG, "SyncCursorPair scan messagesToDelete.add local != remote");
                        messagesToDelete.add((LocalDatabaseMessage) localMessage);
                        // Add remote message
                        saveMessageToAdd(smsToAdd, mmsToAdd, remoteMessage, threadInfoCache);
                    }
                    ...
                    while (localMessage != null &&
                        ...
                        if (matchedLocalMessages.contains(localMessage)) {
                            ...
                            LogUtil.d(LogUtil.BUGLE_ALARM_TAG, "SyncCursorPair scan messagesToDelete.add Duplicate message");
                            messagesToDelete.add((LocalDatabaseMessage) localMessage);
                        } else {
                            matchedLocalMessages.add(localMessage);
                        }
                    ...
                    for (final DatabaseMessage msg : Sets.difference(
                        ...
                        /* Modify by SPRD for bug 548854 Start */
                        if (smsToAdd.size() + mmsToAdd.size() +
                                messagesToDelete.size() < maxMessagesToUpdate) {
                            messagesToDelete.add((LocalDatabaseMessage) msg);
                        }
                        /* Modify by SPRD for bug 548854 End */
                    }
