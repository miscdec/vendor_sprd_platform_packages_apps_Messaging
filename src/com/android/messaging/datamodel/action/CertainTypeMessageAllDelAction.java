...
public class CertainTypeMessageAllDelAction extends Action implements Parcelable {
    ...
    protected Bundle doBackgroundWork() {
        ...
        String subId = type_and_ids.split(",")[4];
        String selfId = "";
        if (messagesId != null && messagesId.length > 0 && Integer.valueOf(subId) != 0) {
            final MessageData message = BugleDatabaseOperations.readMessage(db, messagesId[0]);
            selfId = message.getSelfId();
        }
        deleteCertainTypeMessage(db, box_type, messagesMaxid, maxMmsid, maxSmsid, subId, selfId, getAllConversationIds(db, messagesId));
    ...
    private int deleteCertainTypeMessage(final DatabaseWrapper dbWrapper,  int nType, final String messagesMaxid,
            final String szMaxMmsid, final String szMaxSmsid, String subId, String selfId, ArrayList<String> conversations) {
        String sefIdCondition = TextUtil.isAllWhitespace(selfId) ? "" : " and self_id=" + selfId;
        String szMessageCondition = getMessageStatusFromBugle(nType) +" and _id <= "+ messagesMaxid + sefIdCondition;
        ...
        String subIdCondition = "";
        if (Integer.valueOf(subId) > 0) {
            subIdCondition = " and sub_id=" + subId;
        }

        final ContentResolver resolver = Factory.get().getApplicationContext().getContentResolver();
        String smsCondition = " _id <= "+ szMaxSmsid +" and " + getSmsTypeFromTele(nType) + subIdCondition;
        LogUtil.d(TAG, "delete sms from tele, SQL:"+smsCondition);
        String mmsCondition = " _id <= "+ szMaxMmsid + " and " + getMmsTypeFromTele(nType) + subIdCondition;
