...
public class InsertNewMessageAction extends Action implements Parcelable {
	...
	//613227
    private InsertNewMessageAction(final MessageData message, final int subId, final long alarmTime) {
        super();
        actionParameters.putParcelable(KEY_MESSAGE, message);
        actionParameters.putInt(KEY_SUB_ID, subId);
        actionParameters.putLong(KEY_ALARM_TIME, alarmTime);
    }
	...
	//613227
    private InsertNewMessageAction(final int subId, final String recipients,
            final String messageText, final String subject, final long alarmTime) {
        super();
        if (TextUtils.isEmpty(recipients) || TextUtils.isEmpty(messageText)) {
            Assert.fail("InsertNewMessageAction: Can't have empty recipients or message");
        }
    ...
    protected Object executeAction() {
        ...
        int subId = self.getSubId();/*modify by bug841310 begin*/
        if(!PhoneUtils.getDefault().isActiveSubId(subId)){
            subId =  SmsManager.getDefaultSmsSubscriptionId();
            Log.d(TAG,"[InsertNewMessageAction]===not an Active subId,use defaultSmsSubId:"+subId);
        }/*modify by bug841310 end*/
        //bug 633234 : six queues begin
        if(message.getSubId()!=subId){
            message.bindSubId(subId);
        }

        LogUtil.d(TAG, " message.getSubId "+message.getSubId());
		//613227
        LogUtil.d(LogUtil.BUGLE_ALARM_TAG, " message.getAlarm : "+message.getAlarm());
        ...
        if (isSms) {
            ...
            GlobleUtil.mGropMessagingTotalCount = recipients.size();
            ...
            final MessageData messageToSend = insertSendingMmsMessage(conversationId,
                    message, timestampRoundedToSecond, recipients, subId);
        ...
        if(message.getAlarm() !=0){
		    //TO DO
            MmsUtils.setAlarmForMessage(message.getAlarm());
        }
    ...
    private MessageData createMessage() {
        ...
        if (TextUtils.isEmpty(subjectText)) {
                messageData =MessageData.createDraftSmsMessage(conversationId, self.getId(), messageText);
        } else {
                messageData =MessageData.createDraftMmsMessage(conversationId, self.getId(), messageText,
                    subjectText);
        }
        messageData.bindSubId(subId);
        return messageData;
        //bug 633234 : six queues end
    }
    ...
    private MessageData insertSendingSmsMessage(final MessageData content, final int subId,
        ...
        //613227
        int messageType =  Telephony.Sms.MESSAGE_TYPE_SENT;
        if(content.getAlarm() !=0){
            LogUtil.d(LogUtil.BUGLE_ALARM_TAG, "insertSendingSmsMessage message.getAlarm : "+content.getAlarm());
            messageType = TelephonyEx.SmsEx.MESSAGE_TYPE_ALARM;
        }
        // Insert message into telephony database sms message table
        final Uri messageUri = MmsUtils.insertSmsMessage(context,
                Telephony.Sms.CONTENT_URI,
                ...
                messageType,
                threadId,content.getAlarm() );//613227
        ...
        if (messageUri != null && !TextUtils.isEmpty(messageUri.toString())) {
            ...
            try {
                ...
                //613227
				if(content.getAlarm() !=0){
                    LogUtil.d(TAG, "markMessageAlarm");
                    message.markMessageAlarm();
                    message.setAlarm(content.getAlarm());
                }

                //bug 633234 : six queues begin
                message.bindSubId(subId);
                LogUtil.d(TAG, "content.getSubId():"+content.getSubId());
                LogUtil.d(TAG, "subId:"+subId);
                //bug 633234 : six queues end
                BugleDatabaseOperations.insertNewMessageInTransaction(db, message);

                long sort_timestamp = BugleDatabaseOperations.getSortTimesStampByThreadId(db, threadId);  //only used when sendingConversationId == null
                // Do not update the conversation summary to reflect autogenerated 1:1 messages
                //bug888891 begin
                //if (sendingConversationId != null) {
                if (sendingConversationId != null || sort_timestamp > 0) {
                    //if conversation is auto generated when mutli-sending message, and sort_timestamp in bugle_db is 0
                    sort_timestamp = timestamp;
                }
                BugleDatabaseOperations.updateConversationMetadataInTransaction(db,
                        conversationId, message.getMessageId(), sort_timestamp,
                        false /* senderBlocked */, false /* shouldAutoSwitchSelfId */);
                //bug888891 end
                db.setTransactionSuccessful();
            } finally {
    ...
    private MessageData insertSendingMmsMessage(final String conversationId,
            final MessageData message, final long timestamp,final ArrayList<String> recipients, final int subId) {
        final DatabaseWrapper db = DataModel.get().getDatabase();
        db.beginTransaction();
        ...
        try {
            ...
            final Uri messageUri = MmsUtils.insertSendingMmsMessage(context, recipients,
                        message, subId, self.getNormalizedDestination(), timestamp);
