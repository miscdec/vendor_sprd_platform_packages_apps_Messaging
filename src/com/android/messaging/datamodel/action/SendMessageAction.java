...
public class SendMessageAction extends Action implements Parcelable {
            ...
            if (isSms) {
                ...
                if (recipients.size() == 1) {
                    ...
                    if (LogUtil.isLoggable(TAG, LogUtil.DEBUG)) {
                        LogUtil.d(TAG, "SendMessageAction: Queued SMS messageid:[ " + messageId
                                + "] in conversationId:["+conversationId+"] for sending");
                    }
                    return true;
                } else {
    ...
    protected Bundle doBackgroundWork() {
        ...
        LogUtil.d(TAG, "SendMessageAction: Sending " + (isSms ? "SMS" : "MMS") + " message "
    ...
    private String getSmsServiceCenter(int subId) {
        return actionParameters.getString(KEY_SMS_SERVICE_CENTER);
    }
    ...
    public static boolean updateMessageAndStatus(final boolean isSms, final MessageData message,
        ...
        switch(message.getStatus()) {
            ...
            case MessageData.BUGLE_STATUS_OUTGOING_AWAITING_RETRY:
                //Bug 1011674 begin
                type = Sms.MESSAGE_TYPE_QUEUED;  //MESSAGE_TYPE_SENT->.
                messageBox = Mms.MESSAGE_BOX_OUTBOX;  //MESSAGE_BOX_SENT->.
                //Bug 1011674 end
                break;
        ...
        if (updatedTelephony) {
            //if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
                LogUtil.d(TAG, "SendMessageAction: Updated " + (isSms ? "SMS" : "MMS")
                        + " message " + message.getMessageId()
                        + " in telephony (" + message.getSmsMessageUri() + ") ConversationId:["+message.getConversationId()+"]");
            //}
        } else {
            LogUtil.d(TAG, "SendMessageAction: Failed to update " + (isSms ? "SMS" : "MMS")
                    + " message " + message.getMessageId()
                    + " in telephony (" + message.getSmsMessageUri() + "); ConversationId:["+message.getConversationId()+"]  marking message failed");
        }
        ...
        try {
            if (updatedMessageUri != null) {
                ...
                LogUtil.d(TAG, "SendMessageAction: refreshConversationMetadataInTransaction ConversationId:["+message.getConversationId()+"]");
