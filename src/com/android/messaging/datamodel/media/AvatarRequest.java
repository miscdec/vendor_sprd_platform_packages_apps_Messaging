...
public class AvatarRequest extends UriImageRequest<AvatarRequestDescriptor> {
    ...
    private static LruCache<String, Bitmap> sNetImageCache = new LruCache<String, Bitmap>(11);
    private static final Object netLock = new Object();
    ...
    protected ImageResource loadMediaInternal(List<MediaRequest<ImageResource>> chainedTasks)
        ...
        } else if (MmsConfig.supportSmartSdk()) {
            final String httpUri = mDescriptor.uri.toString();
            if (httpUri.startsWith("http")) {
                bitmap = sNetImageCache.get(mDescriptor.getKey());
                if (null == bitmap) {
                    LogUtil.d(LogUtil.BUGLE_SMART_TAG, "Start Http for image with httpUri: " + httpUri);
                    if (DataModel.get().getServicePortConnectivityUtil().hasNet()) {
                        bitmap = getNetworkImage(httpUri);
                        if (bitmap != null) {
                            LogUtil.d(LogUtil.BUGLE_SMART_TAG, "Network, add cache, getKey:" + mDescriptor.getKey());
                            sNetImageCache.put(mDescriptor.getKey(), bitmap);
                        }
                    } else {
                        ParticipantRefresh.registerRefreshForServerPort(false);
                    }
                }
            }
        }
    ...
    private static final long NETWORKDELAYTIME = 2 * 1000;
    private static long sRtryTimeMillis = 0;

    public Bitmap getNetworkImage(String imageUrl) throws IOException {
        Assert.isNotMainThread();
        InputStream inputStream = null;
        Bitmap bitmap = null;
        HttpURLConnection connection = null;
        synchronized (netLock) {
            if(sRtryTimeMillis > 0){
                if(System.currentTimeMillis() < sRtryTimeMillis){
                    LogUtil.e(LogUtil.BUGLE_SMART_TAG, "Need Delay");
                    return null;
                }
            }
            sRtryTimeMillis = 0;
            try {
                ...
                //Bug 650371 start
                connection.setReadTimeout(25000);//for bug683789
                connection.setConnectTimeout(25000);//for bug683789
                //Bug 650371 end
                connection.connect();
                LogUtil.d(LogUtil.BUGLE_SMART_TAG, "imageUrl/connection.getResponseCode(): "+imageUrl + "/"+ connection.getResponseCode());
                if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                    inputStream = connection.getInputStream();
                    bitmap = BitmapFactory.decodeStream(inputStream);
                }
            } catch (MalformedURLException e) {
                LogUtil.e(LogUtil.BUGLE_SMART_TAG, "MalformedUrl for image with url: "+ imageUrl, e);
            } catch (final OutOfMemoryError e) {
                LogUtil.e(LogUtil.BUGLE_SMART_TAG,"OutOfMemoryError for image with url: " + imageUrl, e);
                Factory.get().reclaimMemory();
            } catch (IOException e) {
                sRtryTimeMillis = System.currentTimeMillis() + NETWORKDELAYTIME;
                bitmap = null;
                LogUtil.e(LogUtil.BUGLE_SMART_TAG,"IOException trying to get inputStream for image with url: "+ imageUrl, e);
            } finally {
                if (inputStream != null) {
                    inputStream.close();
                }
    ...
    /*smart message, begin*/
    public static boolean haveCache(String key){
        return sNetImageCache.get(key) != null;
    }
    /*smart message, end*/
}
