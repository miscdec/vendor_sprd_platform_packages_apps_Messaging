...
public class MediaResourceManager {
    ...
    private static final Executor LOWMEMORY_MEDIA_LOADING_EXECUTOR = Executors.newFixedThreadPool(4);
    ...
    public <T extends RefCountedMediaResource> void requestMediaResourceAsync(
        ...
        String sheapgrowthlimit = MmsConfig.getGrowthLimit();//SystemProperties.get("dalvik.vm.heapgrowthlimit");

        if(sheapgrowthlimit.equals("48m")){
            scheduleAsyncMediaRequest(mediaRequest, LOWMEMORY_MEDIA_LOADING_EXECUTOR);
        }else{
    ...
    public <T extends RefCountedMediaResource> T loadMediaFromCache(
            final MediaRequest<T> mediaRequest) {
    ...
    private <T extends RefCountedMediaResource> void scheduleAsyncMediaRequest(
        ...
        // add bug 832879 start
        try{
            mediaLoadingTask.executeOnExecutor(executor, (Void) null);
        }catch (RejectedExecutionException ex){
            LogUtil.d(LogUtil.BUGLE_TAG, "scheduleAsyncMediaRequest:" + ex);
        }
        // add bug 832879 end
    ...
    <T extends RefCountedMediaResource> void addResourceToMemoryCache(
        ...
        if (mediaCache != null) {
            /*smart message, begin*/
            final String requestKey = mediaRequest.getKey();
            if(requestKey !=null && requestKey.startsWith("http") && !AvatarRequest.haveCache(mediaRequest.getKey())){
                LogUtil.d(LogUtil.BUGLE_SMART_TAG, "addResourceToMemoryCache return, regain logo");
                return;
            }
            /*smart message, end*/
