...
public class SendStatusReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (MESSAGE_SENT_ACTION.equals(action)) {
           ...
           String format  = intent.getStringExtra("format");//for 1249768 cdma or gsm format
            Log.w("SendStatusReceiver","deliver2-----format--"+format);
            final SmsMessageAdapter smsMessage = MmsUtils.getSmsMessageFromDeliveryReport(intent,format);
            ...
            int status = 0;
            try {
                /*modify 1162515 for cdma status report*/
                status = smsMessage.mSmsMessage.getStatus();
                Log.w("SendStatusReceiver","CDMA deliver-----status--"+status+"status>>"+(status>>16));
                if(format.equals(SmsConstants.FORMAT_3GPP2)&&(status!=0)){
                    status=(status>>16)-2;
                }
