...
public class PduParser {
    ...
    public GenericPdu parse() {
        ...
        if ((PduHeaders.MESSAGE_TYPE_SEND_REQ == messageType) ||
            ...
            //489220 begin
            String szValue = new String(mHeaders.getTextString(PduHeaders.CONTENT_TYPE));
            if ("text/plain".equalsIgnoreCase(szValue)) {
                int startPos = mPduDataStream.available();
                byte[] bytes = new byte[1024];
                try {
                    int nLen = mPduDataStream.read(bytes);
                    bytes[nLen] = 0;
                } catch (Exception e) {
                    e.printStackTrace();
                }
                if (bytes == null) {
                    Log.e(LOG_TAG, "parse data bytes: [" + null + "]");
                }
                PduPart pduPart = new PduPart();
                String FILE_NAME = "ReadReport.txt";
                mBody = new PduBody();
                pduPart.setCharset(106);
                pduPart.setFilename(FILE_NAME.getBytes());
                pduPart.setContentLocation(FILE_NAME.getBytes());
                pduPart.setContentId(FILE_NAME.getBytes());
                pduPart.setData(bytes);
                pduPart.setContentType(mHeaders.getTextString(PduHeaders.CONTENT_TYPE));
                mBody.addPart(pduPart);
            } else {
        ...
        switch (messageType) {
            ...
            case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
                ...
                if (ctTypeStr.equals(ContentType.MMS_MULTIPART_MIXED)
                    ...
                    return retrieveConf;
                } else if (ctTypeStr.equals(ContentType.TEXT_PLAIN)) {// by 489220
                    return retrieveConf;
                }
                return null;
