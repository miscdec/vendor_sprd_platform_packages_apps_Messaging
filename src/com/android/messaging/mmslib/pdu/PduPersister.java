...
public class PduPersister {
    ...
    public static final String EXTENDS_TAG_OGG = "ogg";
    public static final String EXTENDS_TAG_AMR = "amr";
    ...
    static {
        ...
        MESSAGE_BOX_MAP.put(MmsEx.Alarmbox.CONTENT_URI, MmsEx.MESSAGE_BOX_ALARM);
            ...
            while (c.moveToNext()) {
                ...
                try {
                    if (ContentType.isDrmType(new String(contentType))) {
                        byte[] dataPathByteArray = getByteArrayFromPartColumn(c, PART_COLUMN_DATA);
                        String dataPath = dataPathByteArray != null ? new String(dataPathByteArray) : null;
                        if (dataPath!=null){
                            String origType = MessagingDrmSession.get().getDrmOrigMimeType(dataPath, new String(contentType));
                            if (origType!=null){
                                if (origType.startsWith("image/")){
                                    part.setContentType("image/dcf".getBytes());
                                }else if (origType.startsWith("audio/")){
                                    part.setContentType("audio/dcf".getBytes());
                                }else if (origType.startsWith("video/")){
                                    part.setContentType("video/dcf".getBytes());
                                }
                            }
                        }
                        //part.setContentType("application/oct-stream".getBytes());
                        Log.d(TAG, "loadParts dataPath is "+dataPath+" contenttype is "+new String(part.getContentType()));
                    }
                }catch(Exception ex){
    ...
    public Uri persistPart(final PduPart part, final long msgId,
        ...
        if (contentType != null) {
            ...
            if (contentType.equals("/") && data != null) {
                if (containsTag(part, EXTENDS_TAG_AMR)) {
                    contentType = ContentType.AUDIO_AMR;
                }
            }
            if (ContentType.APP_OCT.equals(contentType) && data != null) {
                if (containsTag(part, EXTENDS_TAG_OGG)) {
                    contentType = ContentType.AUDIO_OGG;
                } else {
                final String encodedDataString = new EncodedStringValue(
                        charset, data).getString();
                if (encodedDataString != null
                            && encodedDataString.startsWith(BEGIN_VCALENDAR)) {
                        contentType = ContentType.TEXT_VCALENDAR;
                        part.setContentType(contentType.getBytes());
                    } else if (encodedDataString != null
                        && encodedDataString.startsWith(BEGIN_VCARD)) {
            ...
            if ("application/oct-stream".equals(contentType)
                    ||contentType!=null && contentType.endsWith("/dcf")) {
                String name = part.getName()==null?"":new String(part.getName());
                String fileName = part.getFilename()==null?"":new String(part.getFilename());
                String contentLocation = part.getContentLocation()==null?"":new String(part.getContentLocation());
                if (name.endsWith(".dcf")||fileName.endsWith(".dcf")||contentLocation.endsWith(".dcf")) {
                    contentType = "application/vnd.oma.drm.content";
                }
            }

            // bmp
            // Write Here, has no effect on the content type handled above.
            if(ContentType.APP_OCT.equals(contentType)) {
                contentType = ContentType.tryExtractContentTypeFromPduPart(part);
    ...
    private void persistData(final PduPart part, final Uri uri,
        ...
        try {
            ...
            if (ContentType.TEXT_PLAIN.equals(contentType)
                ...
                // uri can look like:
                // content://mms/part/98
                long fileSize = MmsUtils.getMediaFileSize(uri);
                Log.e(TAG, " persist uri is " + uri + " fileSize " + fileSize);
                if (fileSize == 0) {
        ...
        try {
            String drmPath = MessagingDrmSession.get().getPath(uri);
            Log.e(TAG, " persist data path is " + drmPath);
            if (MessagingDrmSession.get().drmCanHandle(drmPath, null)) {
                final ContentValues cv = new ContentValues();
                cv.put(Mms.Part.CONTENT_TYPE, "application/vnd.oma.drm.content");
                Log.e(TAG, " persist drm data update value is " + cv);
                if (mContentResolver.update(uri, cv, null, null) != 1) {
                    Log.e(TAG, " persist data update content drm type failed ");
                }

            }
            Uri scannerUri = (new Uri.Builder()).scheme(ContentResolver.SCHEME_FILE).appendPath(drmPath).build();
            Intent mediaScanner = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, scannerUri);
            mContext.sendBroadcast(mediaScanner);
        }catch(Exception ex) {
        }
    }
    ...
    public Uri persist(final GenericPdu pdu, final Uri uri, final int subId,
        ...
        if (!existingUri && MESSAGE_BOX_MAP.get(uri) == null) {
            throw new MmsException(
                    "Bad destination, must be one of "
                            ...
                            + "content://mms/temp, content://mms/alarmbox."
            );
        }
        synchronized (PDU_CACHE_INSTANCE) {
            // If the cache item is getting updated, wait until it's done updating before
        ...
        for (int i = ENCODED_STRING_COLUMN_NAME_MAP.size(); --i >= 0; ) {
            ...
            if (encodedString != null) {
                ...
                String str = MmsUtils.isSubjectNeedUtf8()?MmsUtils.toUtf8String(toIsoString(encodedString.getTextString())):toIsoString(encodedString.getTextString());
                values.put(ENCODED_STRING_COLUMN_NAME_MAP.valueAt(i),str);
    ...
    private void checkAndLoadToCcRecipients(final HashSet<String> recipients,
        ...
        //modified for bug 618325
        //if (arrayTo != null) {
        LogUtil.d(TAG, "checkAndLoadToCcRecipients --arrayTo="+((arrayTo != null)? arrayTo.toString() : ""));
        if (arrayTo != null && arrayTo.length > 1) {
        ...
        Log.i(TAG, "checkAndLoadToCcRecipients --selfNumber="+selfNumber);
    ...
    public static byte[] getBytes(final String data) {
        try {
            ...
            // Impossible to reach here!
            Log.e(TAG, "ISO_8859_1 or UTF_8 must be supported!", e);
            return new byte[0];
        }
    }
    ...
    public static boolean containsTag(PduPart part, String tag) {
        ...
        if (checkAuido(part.getName(), tag)) {
            return true;
        }
        if (checkAuido(part.getFilename(), tag)) {
            return true;
        }
        if (checkAuido(part.getContentLocation(), tag)) {
            return true;
        }
        if (checkAuido(part.getContentId(), tag)) {
            return true;
        }
        return false;
    }
    private static boolean checkAuido(byte[] partField, String tag) {
        if (partField == null) {
            return false;
        }
        String value = toIsoString(partField).toLowerCase();
        return value != null
                && value.length() > 0
                && value.contains(".")
