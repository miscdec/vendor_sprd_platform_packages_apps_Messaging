...
public class BugleApplication extends Application implements UncaughtExceptionHandler {
    ...
    public static final String CHANNAL_NAME = "Bugle Messaging";
    ...
    public void onCreate() {
        ...
        super.onCreate();
        MmsConfig.checkSmartSdkEnabled();//by smart message
        MmsConfig.setGrowthLimit();
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            ...
            NotificationChannel simNch = notificationManager.getNotificationChannel(PendingIntentConstants.MESSAGING_CHANNAL_ID);
            if (simNch == null) {
                LogUtil.e(TAG, "first create channel");
                NotificationChannel cn = new NotificationChannel(PendingIntentConstants.MESSAGING_CHANNAL_ID,CHANNAL_NAME, NotificationManager.IMPORTANCE_HIGH);
    ...
    private void initSmartSdk() {//by smart message
        try {
            ISmartSdk smartSdkImpl = SmartSdkUtils.createSmartSdk();
            if(smartSdkImpl != null){
                smartSdkImpl.initSdk(getApplicationContext());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
