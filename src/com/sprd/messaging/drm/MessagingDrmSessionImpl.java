...
//                 Toast.makeText(Factory.get().getApplicationContext(), R.string.drm_not_be_selected,
//                 Toast.LENGTH_SHORT).show();
                return false;
            }
            if (!MessagingDrmHelper.isDrmSDFile(mContext, path, null)) {
                Log.d(TAG, "activity -- isSDfile   ");
//                 Toast.makeText(Factory.get().getApplicationContext(), R.string.drm_not_be_shared,
//                 Toast.LENGTH_SHORT).show();
                return false;
            }
        }
...
//             Toast.makeText(Factory.get().getApplicationContext(), R.string.error_in_shared,
//             Toast.LENGTH_SHORT).show();
            break;
        default:
            break;
    ...
    private String mDefaultDrmPath = "/storage/emulated/0";
    @Override
...
//        path = Environment.getInternalStoragePath().getPath();// TCard
        String externalStatus = Environment.getExternalStorageState(); // TCard
        ...
        if (externalStatus.equals(Environment.MEDIA_MOUNTED)) {
            ...
            drmPath = createDRMdownloadDirectory(mDefaultDrmPath);
        }
...
//        scrollView.addView(detailView);
        ...
        //modify for bug 632085 begin
        String fileMessage;
        try {
            fileMessage =getResString(context, res, file, filePath, expirationTime,
                    clickTime, remainTimes, startTime, endTime);

        } catch (Exception e) {
            Log.d(TAG, "error happended when getStrings, the local had changed and can't find the R.id...");
            final Configuration config = res.getConfiguration();
            Locale original = config.locale;
...
//        builder.setView(scrollView);
    ...
    private String getResString(Context context, Resources res, File file, String filePath,String expirationTime,
                                byte[] clickTime, String remainTimes, Long startTime, Long endTime){
        return res.getString(R.string.drm_file_path, file.getParent())
                +"\n"
    ...
    private boolean canTransfer(String filePath) {
        return !(MessagingDrmHelper.isDrmFLFile(mContext,filePath,""));  //bug 1178913
    }
